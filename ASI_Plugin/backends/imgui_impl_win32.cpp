    ...
    if (is_app_focused)
    {
        ...
        if (!io.WantSetMousePos && bd->MouseTrackedArea == 0)
        {
            ...
            if (::GetCursorPos(&pos) && ::ScreenToClient(bd->hWnd, &pos)) {
                float xs = (float)pos.x / (g_ImGuiGlobalScale > 0.f ? g_ImGuiGlobalScale : 1.f);
                float ys = (float)pos.y / (g_ImGuiGlobalScale > 0.f ? g_ImGuiGlobalScale : 1.f);
                io.AddMousePosEvent(xs, ys);
            }
        }
    }
}
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerEx(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, ImGuiIO& io)
{
    ...
    case WM_NCMOUSEMOVE:
    {
        ...
        float xs = (float)mouse_pos.x / (g_ImGuiGlobalScale > 0.f ? g_ImGuiGlobalScale : 1.f);
        float ys = (float)mouse_pos.y / (g_ImGuiGlobalScale > 0.f ? g_ImGuiGlobalScale : 1.f);
        io.AddMousePosEvent(xs, ys);
        return 0;
    }
    case WM_MOUSELEAVE:
