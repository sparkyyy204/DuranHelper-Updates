...
extern void AddLocalSAMPMessage(const char* text);
extern std::string UTF8ToCP1251(const char* utf8);
extern void RunOnMainThread(std::function<void()> func);
...
ImFont* Gui::fontSegoeRegular13 = nullptr;
ImFont* Gui::fontSegoeItalic13 = nullptr;
ImFont* Gui::fontSegoeBoldItalic14 = nullptr;
...
bool Gui::showSearchPopup = false;
char Gui::searchBufferPopup[256] = "";
int Gui::searchMatchIndex = 0;
int Gui::searchMatchCount = 0;
bool Gui::searchPopupFocus = false;
bool Gui::scrollSearch = false;
...
int Gui::scriptToggleKey = 0;
bool Gui::scriptToggleNeedsAlt = false;
bool Gui::scriptToggleNeedsCtrl = false;
bool Gui::scriptToggleNeedsShift = false;
bool Gui::radialActivationToggleMode = false;
...
bool Gui::clearSearchOnClose = false;
bool Gui::closeOnClickOutside = false;
ULONGLONG Gui::lastCloseTime = 0;

bool Gui::quoteEnabled = true;
bool Gui::quoteNotepad = true;
...
extern void OpenChatWithText(const char* text, int cursorOffset = 0);
extern void SendSAMPMessage(const char* msg);
extern std::string UTF8ToCP1251(const char* utf8);

void Gui::LoadSettings() {
    std::wstring path = GetAppDataPath() + L"Settings.json";
    ...
    if (file.is_open()) {
        try {
            ...
            if (j.contains("RadialToggleMode")) radialActivationToggleMode = j["RadialToggleMode"].get<bool>();
            if (j.contains("ScriptToggleKey")) {
                scriptToggleKeyStr = j["ScriptToggleKey"];
                BindItem temp; temp.ahkKey = scriptToggleKeyStr; BinderManager::Get().ParseAhkKey(temp);
                scriptToggleKey = temp.vkCode;
                scriptToggleNeedsAlt = temp.needsAlt;
                scriptToggleNeedsCtrl = temp.needsCtrl;
                scriptToggleNeedsShift = temp.needsShift;
            }
            ...
            if (j.contains("QuoteNotepad")) quoteNotepad = j["QuoteNotepad"].get<bool>();
...
void Gui::ApplyTheme(float alphaMul) {
    ...
    float r, g, b, a;
    // FrameBg Р Р†Р вЂљРІР‚Сњ use C_INPUT
    ...
    // FrameBgHovered Р Р†Р вЂљРІР‚Сњ use C_HOVER
    ...
    // ScrollbarBg Р Р†Р вЂљРІР‚Сњ use C_INPUT
    r = ((C_INPUT >> 0) & 0xFF) / 255.0f; g = ((C_INPUT >> 8) & 0xFF) / 255.0f;
    b = ((C_INPUT >> 16) & 0xFF) / 255.0f; a = ((C_INPUT >> 24) & 0xFF) / 255.0f;
    c[ImGuiCol_ScrollbarBg] = ImVec4(r, g, b, a);

    // Modern flat design aesthetics (ui-ux-pro-max)
...
static std::string CP1251ToUTF8(const std::string& cp1251) {
    ...
    int wlen = MultiByteToWideChar(1251, 0, cp1251.c_str(), -1, NULL, 0);
    if (wlen <= 0) return cp1251;
    std::wstring wstr(wlen, 0);
    MultiByteToWideChar(1251, 0, cp1251.c_str(), -1, &wstr[0], wlen);
    int ulen = WideCharToMultiByte(CP_UTF8, 0, wstr.c_str(), -1, NULL, 0, NULL, NULL);
    if (ulen <= 0) return cp1251;
            ...
            if (rtf[i] == '\'') {
                ...
                if (i + 1 < rtf.length()) {
                    ...
                    try {
                        ...
                        if ((unsigned char)cc < 128) {
                            currentSeg.text += cc;
                        } else {
                            std::string temp; temp += cc;
                            currentSeg.text += CP1251ToUTF8(temp);
                        }
                    } catch(...) {}
                    i += 2;
                }
...
void Gui::LoadLaws() {
    lawSections.clear();
    std::wstring p = GetAppDataPath() + L"laws.json";
    std::ifstream f(p); if (!f.is_open()) return;
    try {
        json j; f >> j;
...
void Gui::LoadFines() {
    fineItems.clear();
    std::wstring p = GetAppDataPath() + L"calculator.json";
    std::ifstream f(p); if (!f.is_open()) {
        // Fallback: try old fines.json
        p = GetAppDataPath() + L"fines.json";
...
void Gui::LoadWanted() {
    ...
    std::ifstream f(p); if (!f.is_open()) {
        ...
        p = GetAppDataPath() + L"wanted.json";
        f.open(p); if (!f.is_open()) return;
    }
    try {
        json j; f >> j;
        ...
        for (auto& item : arr) {
            ...
            if (wi.type == "\xD0\xA3\xD0\x9A \xD0\xA0\xD0\xA4") wi.type = "\xD0\xA3\xD0\x9A"; // РЈРљ
...
static ImFont* AddFontSafe(ImGuiIO& io, const char* path, float size, const ImFontConfig* config, const ImWchar* ranges, float inverseScale) {
    ...
    if (!font) {
        ...
        std::string fp = GetFontsPath() + "arial.ttf";
        font = io.Fonts->AddFontFromFileTTF(fp.c_str(), size, &safeConfig, ranges);
    }
    if (!font) {
        // Fallback 2: Try standard Segoe UI Bold
        std::string fp = GetFontsPath() + "segoeuib.ttf";
        font = io.Fonts->AddFontFromFileTTF(fp.c_str(), size, &safeConfig, ranges);
    }
    if (!font) {
...
void Gui::Init(IDirect3DDevice9* pDevice) {
    ...
    std::string fontsDir = GetFontsPath();
    
    ImFont* baseFont = AddFontSafe(io, (fontsDir + "arial.ttf").c_str(), 16.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic(), 1.0f / rs);
    
    fontArialBlack24 = AddFontSafe(io, (fontsDir + "ariblk.ttf").c_str(), 24.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic(), 1.0f / rs);
    fontSegoeBold12 = AddFontSafe(io, (fontsDir + "segoeuib.ttf").c_str(), 12.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic(), 1.0f / rs);
    fontSegoeBold14 = AddFontSafe(io, (fontsDir + "segoeuib.ttf").c_str(), 14.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic(), 1.0f / rs);
    fontSegoeBold20 = AddFontSafe(io, (fontsDir + "segoeuib.ttf").c_str(), 20.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic(), 1.0f / rs);
    
    fontSegoeRegular13 = AddFontSafe(io, (fontsDir + "segoeui.ttf").c_str(), 13.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic(), 1.0f / rs);
    fontSegoeItalic13 = AddFontSafe(io, (fontsDir + "segoeuii.ttf").c_str(), 13.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic(), 1.0f / rs);
    fontSegoeBoldItalic14 = AddFontSafe(io, (fontsDir + "segoeuiz.ttf").c_str(), 14.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic(), 1.0f / rs);

    fontSegoeBlack32 = io.Fonts->AddFontFromFileTTF((fontsDir + "seguibl.ttf").c_str(), 28.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic());
    if (!fontSegoeBlack32) {
        fontSegoeBlack32 = io.Fonts->AddFontFromFileTTF((fontsDir + "segoeuib.ttf").c_str(), 28.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic());
    }
    if (!fontSegoeBlack32) {
        fontSegoeBlack32 = io.Fonts->AddFontFromFileTTF((fontsDir + "arial.ttf").c_str(), 28.0f * rs, &fc, io.Fonts->GetGlyphRangesCyrillic());
    }
    if (!fontSegoeBlack32) {
...
void Gui::Toggle() {
    ...
    show = !show;
    if (!show) {
        lastCloseTime = GetTickCount64();
    }
    if (show) {
        showBinderHint = false; // Exclusivity
        float cx = *(float*)0xB6EC1C;
        float cy = *(float*)0xB6EC18;
        if (cx > 0.0001f || cx < -0.0001f) savedSensX = cx;
        if (cy > 0.0001f || cy < -0.0001f) savedSensY = cy;
        if (clearSearchOnClose) clearNextFrame = true; // Signal Render() to aggressively clear text inputs
        overlayErrorTimer = 0.0f; // Clear errors on toggle
    } else {
        if (!rememberTab) {
            activeTab = -1;      // Reset to Menu or Laws tab
            showSettings = false;  // Also close settings panel
        }
        showLawDropdown = false;
        SetCursor(NULL);
        
        if (windowDraggable) SaveSettings();
...
void Gui::HandleEscape() {
    if (true && (activeTab != -1 || showSettings)) {
        activeTab = -1;
        showSettings = false;
        showLawDropdown = false;
        SaveSettings();
...
void Gui::ToggleBinderHint() {
    ...
    if (GetTickCount64() - lastToggle < 300) return;
    lastToggle = GetTickCount64();

    if (!showBinderHint) {
        // We are opening the hint. If the overlay is open, close it first.
        if (show) Toggle();
        showBinderHint = true;
    } else {
        // We are closing the hint.
        showBinderHint = false;
    }
}
...
void Gui::DrawHudFrame(ImDrawList* dl, ImVec2 o) {
    ...
    // Gold accent top-left вЂ” offset 1px outward so line sits above edge, no inner glow
    ...
    dl->AddText(fontArialBlack24, 24.0f, ImVec2(startX, o.y+10), C_WHITE, "DURAN");
    dl->AddText(fontArialBlack24, 24.0f, ImVec2(startX+duranW, o.y+10), C_GOLD, "HELPER");
...
void Gui::DrawTabs(ImDrawList* dl, ImVec2 o) {
    ...
    // Settings gear icon вЂ” right of tabs
...
void Gui::DrawGridBackButton(ImDrawList* dl, ImVec2 o) {
    float bx = o.x + 20;
    float by = o.y + 12;
    ImGui::SetCursorScreenPos(ImVec2(bx, by));
    if (ImGui::InvisibleButton("##gridBackBtn", ImVec2(100, 26))) {
        activeTab = -1;
        showSettings = false;
        showLawDropdown = false;
        SaveSettings();
    ...
    bool hover = ImGui::IsItemHovered();
    ImU32 btnBg = hover ? IM_COL32(255,255,255,10) : C_BOX;
    ImU32 textCol = hover ? C_WHITE : C_GRAY;
    
    dl->AddRectFilled(ImVec2(bx, by), ImVec2(bx+100, by+26), btnBg, 4.0f);
    dl->AddRect(ImVec2(bx, by), ImVec2(bx+100, by+26), C_LINE, 4.0f);
    
    // Back Arrow
    dl->AddLine(ImVec2(bx+15, by+13), ImVec2(bx+25, by+7), textCol, 2.0f);
    dl->AddLine(ImVec2(bx+15, by+13), ImVec2(bx+25, by+19), textCol, 2.0f);
    dl->AddLine(ImVec2(bx+15, by+13), ImVec2(bx+35, by+13), textCol, 2.0f);
    
    dl->AddText(fontSegoeBold12, 11.0f, ImVec2(bx+42, by+6), textCol, "\xD0\x9D\xD0\x90\xD0\x97\xD0\x90\xD0\x94");
}
...
void Gui::DrawGridMenu(ImDrawList* dl, ImVec2 o) {
    ...
    for(int i=0; i<4; i++) dl->AddLine(z1[i], z1[i+1], goldAcc, 1.5f);
    dl->AddCircleFilled(z1[1], 4.0f, goldAcc);
    dl->AddCircleFilled(z1[3], 4.0f, goldAcc);
    
    // Zigzag 2 (Bottom chaotic line)
    ImVec2 z2[5] = {ImVec2(o.x+75, o.y+329), ImVec2(o.x+187, o.y+259), ImVec2(o.x+337, o.y+294), ImVec2(o.x+487, o.y+224), ImVec2(o.x+637, o.y+273)};
    for(int i=0; i<4; i++) dl->AddLine(z2[i], z2[i+1], goldAcc, 1.5f);
    ...
    struct GridBtn {
        ...
        const char* title;
        float x; float y;
        void (*drawIcon)(ImDrawList*, float, float, ImU32);
    };
    ...
    if (showTabLaws) btns.push_back({0, "\xD0\x97\xD0\x90\xD0\x9A\xD0\x9E\xD0\x9D\xD0\xAB", 0, 0, drawLawsIcon});
    ...
    if (count == 4) {
        float startY = 432.0f / 2.0f - (2 * 120.0f + 25.0f) / 2.0f;
        btns[0].x = (700.0f - (2 * 140.0f + 40.0f)) / 2.0f; btns[0].y = startY;
        btns[1].x = btns[0].x + 180.0f; btns[1].y = startY;
        btns[2].x = btns[0].x; btns[2].y = startY + 145.0f;
    ...
    for (size_t i=0; i<btns.size(); i++) {
        ...
        ImGui::SetCursorScreenPos(ImVec2(bx, by));
        char tid[16]; sprintf_s(tid, "##gridBtn%d", i);
        if (ImGui::InvisibleButton(tid, ImVec2(bw, bh))) {
            if (btns[i].id == -2) {
                showSettings = true;
            } else {
                activeTab = btns[i].id;
        ...
        ImU32 bgCol = hover ? C_HOVER : C_BG;
        ImU32 borderCol = active ? C_GOLD : (hover ? C_LINE : C_BORDER);
        ImU32 iconCol = hover ? C_GOLD : C_GRAY;
        ImU32 textCol = hover ? C_WHITE : C_GRAY;

        ImVec2 poly[] = {
            {bx, by+cr}, {bx+cr, by}, {bx+bw-cr, by}, {bx+bw, by+cr},
            {bx+bw, by+bh-cr}, {bx+bw-cr, by+bh}, {bx+cr, by+bh}, {bx, by+bh-cr}
        };
        dl->AddConvexPolyFilled(poly, 8, bgCol);
    ...
    std::string profileLabel = "\xD0\x9F\xD1\x80\xD0\xBE\xD1\x84\xD0\xB8\xD0\xBB\xD1\x8C: ";
    std::string profileValue = BinderManager::Get().currentProfileName;
    std::string separator = "   |   ";
    
    ImVec2 dateSz = fontSegoeBold14->CalcTextSizeA(14.0f, FLT_MAX, 0.0f, dateBuf);
    ImVec2 sepSz = fontSegoeBold14->CalcTextSizeA(14.0f, FLT_MAX, 0.0f, separator.c_str());
    ImVec2 plabSz = fontSegoeBold14->CalcTextSizeA(14.0f, FLT_MAX, 0.0f, profileLabel.c_str());
    ImVec2 pvalSz = fontSegoeBold14->CalcTextSizeA(14.0f, FLT_MAX, 0.0f, profileValue.c_str());
    
    float totalW = dateSz.x + sepSz.x + plabSz.x + pvalSz.x;
    float cutH = 30.0f;
    float cutW = totalW + 40.0f; // Width at the top flat part of trapezoid
    float centerX = o.x + W/2.0f;
    float bottomY = o.y + H; // Exact bottom of the window
    
    // Trapezoid cutout perfectly matching bottom border (CCW order for correct ImGui anti-aliasing)
    // We bring the bottom Y back to 'bottomY' so the main window border can serve as the bottom edge,
    // exactly like how the top bar works.
    ImVec2 fillPts[4] = {
        ImVec2(centerX + cutW/2 + cutH - 1.0f, bottomY - 1.0f), // Bottom right
        ImVec2(centerX + cutW/2, bottomY - cutH),        // Top right
        ImVec2(centerX - cutW/2, bottomY - cutH),        // Top left
        ImVec2(centerX - cutW/2 - cutH + 1.0f, bottomY - 1.0f)  // Bottom left
    };
    // Draw the background of the trapezoid using C_HEADER so it matches the top bar perfectly
    dl->AddConvexPolyFilled(fillPts, 4, C_HEADER);
    
    // Draw the border. We use 'false' for open to not double-draw the bottom line,
    // avoiding the thickness bug. The main window border will show at the bottom perfectly.
    ImVec2 borderPts[4] = {
        ImVec2(centerX - cutW/2 - cutH, bottomY),
        ImVec2(centerX - cutW/2, bottomY - cutH),
        ImVec2(centerX + cutW/2, bottomY - cutH),
        ImVec2(centerX + cutW/2 + cutH, bottomY)
    };
    
    // Inner main window border
    // Draw lines individually to balance anti-aliased visual thickness
    dl->AddLine(borderPts[0], borderPts[1], C_LINE, 1.5f); // Left slanted
    dl->AddLine(borderPts[1], borderPts[2], C_LINE, 2.0f); // Top horizontal
    dl->AddLine(borderPts[2], borderPts[3], C_LINE, 1.5f); // Right slanted
    
    
    float textY = bottomY - cutH + (cutH - 14.0f) / 2.0f - 1.0f;
    float cx = centerX - totalW / 2.0f;
    
    dl->AddText(fontSegoeBold14, 14.0f, ImVec2(cx, textY), C_GRAY, dateBuf);
    cx += dateSz.x;
    
    dl->AddText(fontSegoeBold14, 14.0f, ImVec2(cx, textY), IM_COL32(80, 80, 80, 255), separator.c_str());
    cx += sepSz.x;
    
    dl->AddText(fontSegoeBold14, 14.0f, ImVec2(cx, textY), C_GRAY, profileLabel.c_str());
    cx += plabSz.x;
    
    dl->AddText(fontSegoeBold14, 14.0f, ImVec2(cx, textY), C_GOLD, profileValue.c_str());
}
...
void Gui::RenderLawsTab(ImDrawList* dl, ImVec2 o) {
    ...
    if (lawSections.empty()) {
        float icx = o.x + 350, icy = o.y + 210; float cx = icx, cy = icy;
        ImU32 col = C_GRAY;
        dl->AddLine(ImVec2(cx-36, cy+30), ImVec2(cx-36, cy-30), col, 3.0f);
        dl->AddLine(ImVec2(cx+36, cy+30), ImVec2(cx+36, cy-30), col, 3.0f);
        dl->AddLine(ImVec2(cx, cy+36), ImVec2(cx, cy-24), col, 3.0f);
        dl->AddLine(ImVec2(cx-36, cy-30), ImVec2(cx-16, cy-30), col, 3.0f);
        dl->AddLine(ImVec2(cx-16, cy-30), ImVec2(cx, cy-24), col, 3.0f);
        dl->AddLine(ImVec2(cx+36, cy-30), ImVec2(cx+16, cy-30), col, 3.0f);
        dl->AddLine(ImVec2(cx+16, cy-30), ImVec2(cx, cy-24), col, 3.0f);
        dl->AddLine(ImVec2(cx-36, cy+30), ImVec2(cx-16, cy+30), col, 3.0f);
        dl->AddLine(ImVec2(cx-16, cy+30), ImVec2(cx, cy+36), col, 3.0f);
        dl->AddLine(ImVec2(cx+36, cy+30), ImVec2(cx+16, cy+30), col, 3.0f);
        dl->AddLine(ImVec2(cx+16, cy+30), ImVec2(cx, cy+36), col, 3.0f);
        dl->AddLine(ImVec2(cx-28, cy-10), ImVec2(cx-8, cy-4), col, 3.0f);
        dl->AddLine(ImVec2(cx-28, cy+10), ImVec2(cx-8, cy+16), col, 3.0f);
        dl->AddLine(ImVec2(cx+28, cy-10), ImVec2(cx+8, cy-4), col, 3.0f);
        dl->AddLine(ImVec2(cx+28, cy+10), ImVec2(cx+8, cy+16), col, 3.0f);

        const char* emtTxt = "\xD0\x91\xD0\x90\xD0\x97\xD0\x90 \xD0\x97\xD0\x90\xD0\x9A\xD0\x9E\xD0\x9D\xD0\x9E\xD0\x92 \xD0\x9F\xD0\xA3\xD0\xA1\xD0\xA2\xD0\x90";
        ImVec2 t1 = fontSegoeBold20->CalcTextSizeA(20.0f, FLT_MAX, 0.0f, emtTxt);
        dl->AddText(fontSegoeBold20, 20.0f, ImVec2(icx - t1.x/2, icy+85), C_GRAY, emtTxt);

        const char* emtSub = "\xD0\x9E\xD1\x82\xD0\xBA\xD1\x80\xD0\xBE\xD0\xB9\xD1\x82\xD0\xB5 \xD0\xBB\xD0\xB0\xD1\x83\xD0\xBD\xD1\x87\xD0\xB5\xD1\x80 \xD0\xB8 \xD0\xBD\xD0\xB0\xD1\x81\xD1\x82\xD1\x80\xD0\xBE\xD0\xB9\xD1\x82\xD0\xB5 \xD0\xBF\xD1\x80\xD0\xBE\xD1\x84\xD0\xB8\xD0\xBB\xD1\x8C \xD1\x81 \xD0\xA3\xD0\x9A / \xD0\x9A\xD0\xBE\xD0\x90\xD0\x9F."; 
        ImVec2 t2 = fontSegoeBold14->CalcTextSizeA(14.0f, FLT_MAX, 0.0f, emtSub);
        dl->AddText(fontSegoeBold14, 14.0f, ImVec2(icx - t2.x/2, icy+115), IM_COL32(92,99,112,255), emtSub);
        return;
    }
    ...
    float artY = o.y + 115;
    float artH = H - 110 - 15;
    float colW = 325.0f; // Adjusted to fill the space
    ...
    // Set width to 674 so that o.x + 20 + 674 = o.x + 694 (matches Settings right edge exactly)
    ImGui::BeginChild("##LawsScroll", ImVec2(674, artH), false, ImGuiWindowFlags_NoBackground);
    if (resetLawsScroll) {
    ...
    bool isTextSelected = (selectedLawSection >= 0 && selectedLawSection < (int)lawSections.size() && lawSections[selectedLawSection].type == "text");

    if (!sq.empty() && !isTextSelected) {
        ...
        for (int s = startSec; s < endSec; s++) {
            ...
            for (auto& law : sec.items) {
                if (law.type == "head") {
                    ...
                    // Check if search matches the head itself Р Р†Р вЂљРІР‚Сњ if so, show all items under it
                    if (ToLowerUTF8(law.txt).find(sq) != std::string::npos) {
                        ...
                        if (lastPrintedHeader != currentHeader) {
                            ...
                            cdl->AddLine(ImVec2(cp.x + labelW + 8, sy + 7), ImVec2(cp.x + W - 50, sy + 7), C_BORDER, 1.0f);
                            yOff += 22.0f;
                ...
                if (lastPrintedHeader != currentHeader) {
                    float sy = cp.y + yOff;
                    std::string srcLabel = "\xD0\x98\xD0\xA1\xD0\xA2\xD0\x9E\xD0\xA7\xD0\x9D\xD0\x98\xD0\x9A: " + sec.name; // "Р ВР РЋР СћР С›Р В§Р СњР ВР С™: "
                ...
                // Overlay matching substrings in gold on top (no layout impact)
                {
                    ...
                    while (segStart <= law.txt.length()) {
                        ...
                        // Current segment: [segStart, nlPos) Р Р†Р вЂљРІР‚Сњ one \n-delimited line
                        ...
                        if (segStart < nlPos) {
                            ...
                            // Empty line (standalone \n) Р Р†Р вЂљРІР‚Сњ just advance Y
                            lineY += lineH;
                        }
        ...
        if (sec.type == "text") {
            ...
            for (const auto& seg : sec.noteSegments) {
                ImFont* f = fontSegoeBold14;
                float fSize = 15.0f;
                if (seg.bold && seg.italic) { f = fontSegoeBoldItalic14 ? fontSegoeBoldItalic14 : fontSegoeBold14; fSize = 16.0f; }
                else if (seg.bold) { f = fontSegoeBold14; fSize = 16.0f; }
                else if (seg.italic) { f = fontSegoeItalic13 ? fontSegoeItalic13 : fontSegoeBold14; fSize = 15.0f; }
                ...
                while (start < seg.text.length()) {
                    ...
                    if (!word.empty()) {
                        ...
                        if (wordW > maxW) {
                            // Word is longer than the entire line, split it by characters
                            std::string currentPart = "";
                            float partW = 0.0f;
                            for (size_t i = 0; i < word.length(); ++i) {
                                std::string ch = word.substr(i, 1);
                                // Handle UTF-8 multi-byte characters safely
                                if ((word[i] & 0xC0) == 0xC0) {
                                    size_t charLen = 1;
                                    while (i + charLen < word.length() && (word[i + charLen] & 0xC0) == 0x80) charLen++;
                                    ch = word.substr(i, charLen);
                                    i += charLen - 1;
                                }
                                float chW = f->CalcTextSizeA(fSize, FLT_MAX, 0.0f, ch.c_str()).x;
                                if (currentLine.totalW + partW + chW > maxW) {
                                    if (!currentPart.empty()) {
                                        currentLine.cmds.push_back({currentPart, f, fSize, c, seg.underline, partW});
                                        currentLine.totalW += partW;
                                    }
            ...
            if (quoteEnabled && quoteNotepad && ImGui::IsWindowHovered()) {
                if (ImGui::IsMouseClicked(0)) {
                    int clicks = ImGui::GetIO().MouseClickedCount[0];
                    if (clicks == 3) {
                        // Triple click: select line
                        selStart = ImVec2(0.0f, mPos.y - startY);
                        selEnd = ImVec2(9999.0f, mPos.y - startY);
                        isSelecting = false;
                        hasSelection = true;
                    } else if (clicks == 2) {
                        // Double click: select word
                        selStart = ImVec2(mPos.x, mPos.y - startY);
                        selEnd = selStart;
                        isSelecting = false;
                        hasSelection = true;
                    } else {
            ...
            if (isSelecting) {
                selEnd = ImVec2(mPos.x, mPos.y - startY);
                
                ImVec2 winPos = ImGui::GetWindowPos();
                ImVec2 winSize = ImGui::GetWindowSize();
                if (mPos.y < winPos.y + 20.0f) {
                    ImGui::SetScrollY(ImGui::GetScrollY() - 15.0f);
                } else if (mPos.y > winPos.y + winSize.y - 20.0f) {
                    ImGui::SetScrollY(ImGui::GetScrollY() + 15.0f);
                }
            ...
            for (auto& l : lines) {
                ...
                float curX = 0;
                bool lineHadSelection = false;
                for (auto& cmd : l.cmds) {
                    cdl_notes->PushTextureID(cmd.font->OwnerAtlas->TexID);
                    
                    // Draw full text first
                    cdl_notes->AddText(cmd.font, cmd.fontSize, ImVec2(startX + curX, startY + curY), cmd.color, cmd.text.c_str());
                    if (cmd.underline) {
                        cdl_notes->AddLine(ImVec2(startX + curX, startY + curY + cmd.fontSize + 1.0f), ImVec2(startX + curX + cmd.w, startY + curY + cmd.fontSize + 1.0f), cmd.color, 1.0f);
                    }
                    ...
                    // Draw highlights OVER the text
                    if (searchLaws[0] != '\0') {
                        std::string lowerText = ToLowerUTF8(cmd.text);
                        std::string lowerQuery = ToLowerUTF8(searchLaws);
                        size_t matchPos = lowerText.find(lowerQuery);
                        while (matchPos != std::string::npos) {
                            std::string pre = cmd.text.substr(0, matchPos);
                            std::string matchStr = cmd.text.substr(matchPos, lowerQuery.length());
                            float preW = cmd.font->CalcTextSizeA(cmd.fontSize, FLT_MAX, 0.0f, pre.c_str()).x;
                            float matchW = cmd.font->CalcTextSizeA(cmd.fontSize, FLT_MAX, 0.0f, matchStr.c_str()).x;
                            
                            ImU32 highlightCol = C_GOLD_BG;
                            ImU32 textCol = C_GOLD;
                            if (currentMatchCount == searchMatchIndex) {
                                highlightCol = C_GOLD;
                                textCol = C_DARK;
                                targetScrollY = curY - 50.0f;
                            }
                            
                            cdl_notes->AddRectFilled(ImVec2(startX + curX + preW, startY + curY - 1), ImVec2(startX + curX + preW + matchW, startY + curY + 14.0f), highlightCol, 2.0f);
                            cdl_notes->AddText(cmd.font, cmd.fontSize, ImVec2(startX + curX + preW, startY + curY), textCol, matchStr.c_str());
                            
                            currentMatchCount++;
                            matchPos = lowerText.find(lowerQuery, matchPos + lowerQuery.length());
                        }
                    }
                    
                    cdl_notes->PopTextureID();
                    curX += cmd.w;
                }
                if (lineHadSelection) {
                    currentSelection += "\n";
            ...
            if (quoteEnabled && quoteNotepad && hasSelection && ImGui::IsWindowHovered() && ImGui::IsMouseClicked(1)) {
                ImGui::OpenPopup("NotepadQuoteMenu");
            }
            ImGui::SetNextWindowSize(ImVec2(140, 28));
            ImGui::PushStyleColor(ImGuiCol_PopupBg, IM_COL32(0, 0, 0, 0));
            ImGui::PushStyleColor(ImGuiCol_Border, IM_COL32(0, 0, 0, 0));
            ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f);
            ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0.0f, 0.0f));
            if (ImGui::BeginPopup("NotepadQuoteMenu")) {
                ImVec2 ppos = ImGui::GetWindowPos();
                ImDrawList* pdl = ImGui::GetWindowDrawList();
                
                bool hover = ImGui::IsMouseHoveringRect(ppos, ImVec2(ppos.x + 140, ppos.y + 28));
                pdl->AddRectFilled(ppos, ImVec2(ppos.x + 140, ppos.y + 28), C_BOX, 14.0f);
                pdl->AddRect(ppos, ImVec2(ppos.x + 140, ppos.y + 28), C_BORDER, 14.0f, 0, 1.5f);
                
                ImVec2 ts = fontSegoeBold14->CalcTextSizeA(14.0f, FLT_MAX, 0.0f, "\xD0\x9F\xD0\xA0\xD0\x9E\xD0\xA6\xD0\x98\xD0\xA2\xD0\x98\xD0\xA0\xD0\x9E\xD0\x92\xD0\x90\xD0\xA2\xD0\xAC"); // "РџР РћР¦РРўРР РћР’РђРўР¬"
                pdl->AddText(fontSegoeBold14, 14.0f, ImVec2(ppos.x + 70 - ts.x/2, ppos.y + 14 - ts.y/2), hover ? C_GOLD : C_WHITE, "\xD0\x9F\xD0\xA0\xD0\x9E\xD0\xA6\xD0\x98\xD0\xA2\xD0\x98\xD0\xA0\xD0\x9E\xD0\x92\xD0\x90\xD0\xA2\xD0\xAC");
                
                if (ImGui::InvisibleButton("##qbtn", ImVec2(140, 28))) {
                    ExecuteLawQuote(selectedText);
                    hasSelection = false;
            ...
            searchMatchCount = currentMatchCount;
            if (searchMatchCount > 0) {
                if (searchMatchIndex >= searchMatchCount) searchMatchIndex = 0;
                if (searchMatchIndex < 0) searchMatchIndex = searchMatchCount - 1;
            } else {
                searchMatchIndex = 0;
            }
            
            if (scrollSearch && targetScrollY >= 0.0f) {
            ...
            auto drawItem1col = [&](const LawItem& law, float x, float& yOff) {
                ...
                if (law.type == "head") {
                    currentHeader = law.txt;
                    ImVec2 txtSz = fontSegoeBold12->CalcTextSizeA(12.0f, FLT_MAX, fullColW, law.txt.c_str());
                    cdl_notes->AddText(fontSegoeBold12, 12.0f, ImVec2(x, y), C_GOLD, law.txt.c_str(), nullptr, fullColW);
                    cdl_notes->AddLine(ImVec2(x, y + txtSz.y + 4.0f), ImVec2(x+fullColW, y + txtSz.y + 4.0f), C_BORDER, 1.5f);
                    
                    if (quoteEnabled && quoteChapter) {
                        ImVec2 pMin(x - 5.0f, y);
                        ImVec2 pMax(x + fullColW + 5.0f, y + txtSz.y + 4.0f);
                        if (ImGui::IsWindowHovered() && ImGui::IsMouseHoveringRect(pMin, pMax, false)) {
                            ImVec2 wp = ImGui::GetWindowPos(); ImVec2 ws = ImGui::GetWindowSize();
                    ...
                    if (!law.id.empty()) {
                        cdl_notes->AddText(fontSegoeBold14, 13.0f, ImVec2(x, blockCenterY - 6.0f), C_GOLD, law.id.c_str());
                    }
                    cdl_notes->AddText(fontSegoeBold14, 13.0f, ImVec2(x+idW, y), C_WHITE, law.txt.c_str(), nullptr, maxTxtW);
                    if (!law.pun.empty()) {
                        ...
                        float lineEndX = punX - 8.0f;
                        if (lineStartX < lineEndX) {
                            DrawDashedLine(cdl_notes, ImVec2(lineStartX, blockCenterY), ImVec2(lineEndX, blockCenterY), IM_COL32(255,255,255,50), 4, 4);
                        }
                        
                        cdl_notes->AddText(fontSegoeBold14, 12.0f, ImVec2(punX, punY), C_RED, law.pun.c_str());
                    }
                    yOff += (txtSz.y > 15.0f ? txtSz.y + 10.0f : 20.0f);
                }
            };
            ...
            // Split items by col: col=1 (or 0) Р Р†РІР‚В РІР‚в„ў left, col=2 Р Р†РІР‚В РІР‚в„ў right
            ...
            auto drawItem = [&](const LawItem& law, float x, float& yOff) {
                ...
                if (law.type == "head") {
                    ImVec2 txtSz = fontSegoeBold12->CalcTextSizeA(12.0f, FLT_MAX, colW, law.txt.c_str());
                    cdl->AddText(fontSegoeBold12, 12.0f, ImVec2(x, y), C_GOLD, law.txt.c_str(), nullptr, colW);
                    cdl->AddLine(ImVec2(x, y + txtSz.y + 4.0f), ImVec2(x+colW, y + txtSz.y + 4.0f), C_BORDER, 1.5f);
                    
                    if (quoteEnabled && quoteChapter) {
                        ImVec2 pMin(x - 5.0f, y);
                        ImVec2 pMax(x + colW + 5.0f, y + txtSz.y + 4.0f);
                        if (ImGui::IsWindowHovered() && ImGui::IsMouseHoveringRect(pMin, pMax, false)) {
                            ImVec2 wp = ImGui::GetWindowPos(); ImVec2 ws = ImGui::GetWindowSize();
                            cdl->PushClipRect(ImVec2(wp.x - 5.0f, wp.y), ImVec2(wp.x + ws.x, wp.y + ws.y), false);
    ...
    if (searchLaws[0] != '\0' && searchMatchCount > 0) {
        float pW = 100.0f;
        float pH = 28.0f;
        ImVec2 pPos(o.x + W - pW - 25.0f, o.y + H - pH - 25.0f);
        
        ImGui::SetNextWindowPos(pPos);
        ImGui::SetNextWindowSize(ImVec2(pW, pH));
        ImGui::Begin("##SearchOverlay", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoScrollWithMouse);
        
        ImDrawList* fg_dl = ImGui::GetWindowDrawList();
        
        // Background and border matching search bar theme
        fg_dl->AddRectFilled(pPos, ImVec2(pPos.x + pW, pPos.y + pH), C_BG, 6.0f);
        fg_dl->AddRectFilled(pPos, ImVec2(pPos.x + pW, pPos.y + pH), C_HEADER, 6.0f);
        fg_dl->AddRect(pPos, ImVec2(pPos.x + pW, pPos.y + pH), C_BORDER, 6.0f, 0, 1.5f);
        
        char statTxt[32];
        sprintf_s(statTxt, "%d/%d", searchMatchCount > 0 ? searchMatchIndex + 1 : 0, searchMatchCount);
        ImVec2 statSz = fontSegoeBold14->CalcTextSizeA(14.0f, FLT_MAX, 0.0f, statTxt);
        fg_dl->AddText(fontSegoeBold14, 14.0f, ImVec2(pPos.x + 10, pPos.y + (pH - statSz.y) / 2.0f), C_WHITE, statTxt);
        ...
        // Up Arrow
        ImVec2 btnUpPos(pPos.x + pW - 46, pPos.y + (pH - 20) / 2.0f);
        ImGui::SetCursorScreenPos(btnUpPos);
        if (ImGui::Button("##prevM", ImVec2(20, 20))) {
            searchMatchIndex--;
            scrollSearch = true;
        }
        if (ImGui::IsItemHovered()) fg_dl->AddCircleFilled(ImVec2(btnUpPos.x + 10, btnUpPos.y + 10), 10.0f, IM_COL32(255, 255, 255, 15));
        fg_dl->AddLine(ImVec2(btnUpPos.x + 6, btnUpPos.y + 12), ImVec2(btnUpPos.x + 10, btnUpPos.y + 8), C_GRAY, 2.0f);
        fg_dl->AddLine(ImVec2(btnUpPos.x + 10, btnUpPos.y + 8), ImVec2(btnUpPos.x + 14, btnUpPos.y + 12), C_GRAY, 2.0f);

        // Down Arrow
        ImVec2 btnDnPos(pPos.x + pW - 24, pPos.y + (pH - 20) / 2.0f);
        ImGui::SetCursorScreenPos(btnDnPos);
        if (ImGui::Button("##nextM", ImVec2(20, 20))) {
            searchMatchIndex++;
            scrollSearch = true;
        }
        if (ImGui::IsItemHovered()) fg_dl->AddCircleFilled(ImVec2(btnDnPos.x + 10, btnDnPos.y + 10), 10.0f, IM_COL32(255, 255, 255, 15));
        fg_dl->AddLine(ImVec2(btnDnPos.x + 6, btnDnPos.y + 8), ImVec2(btnDnPos.x + 10, btnDnPos.y + 12), C_GRAY, 2.0f);
        fg_dl->AddLine(ImVec2(btnDnPos.x + 10, btnDnPos.y + 12), ImVec2(btnDnPos.x + 14, btnDnPos.y + 8), C_GRAY, 2.0f);
...
void Gui::RenderFinesTab(ImDrawList* dl, ImVec2 o) {
    ...
    if (fineItems.empty()) {
        float icx = o.x + 350, icy = o.y + 210; float cx = icx, cy = icy;
        ImU32 col = C_GRAY;
        dl->AddLine(ImVec2(cx-28, cy-40), ImVec2(cx+8, cy-40), col, 3.0f);
        dl->AddLine(ImVec2(cx+8, cy-40), ImVec2(cx+28, cy-20), col, 3.0f);
        dl->AddLine(ImVec2(cx+28, cy-20), ImVec2(cx+28, cy+40), col, 3.0f);
        dl->AddLine(ImVec2(cx+28, cy+40), ImVec2(cx-28, cy+40), col, 3.0f);
        dl->AddLine(ImVec2(cx-28, cy+40), ImVec2(cx-28, cy-40), col, 3.0f);
        dl->AddLine(ImVec2(cx+8, cy-40), ImVec2(cx+8, cy-20), col, 3.0f);
        dl->AddLine(ImVec2(cx+8, cy-20), ImVec2(cx+28, cy-20), col, 3.0f);
        dl->AddLine(ImVec2(cx-16, cy-10), ImVec2(cx+16, cy-10), col, 3.0f);
        dl->AddLine(ImVec2(cx-16, cy+4), ImVec2(cx+16, cy+4), col, 3.0f);
        dl->AddLine(ImVec2(cx-16, cy+18), ImVec2(cx+4, cy+18), col, 3.0f);

        const char* emtTxt = "\xD0\x91\xD0\x90\xD0\x97\xD0\x90 \xD0\xA8\xD0\xA2\xD0\xA0\xD0\x90\xD0\xA4\xD0\x9E\xD0\x92 \xD0\x9F\xD0\xA3\xD0\xA1\xD0\xA2\xD0\x90";
        ImVec2 t1 = fontSegoeBold20->CalcTextSizeA(20.0f, FLT_MAX, 0.0f, emtTxt);
        dl->AddText(fontSegoeBold20, 20.0f, ImVec2(icx - t1.x/2, icy+85), C_GRAY, emtTxt);

        const char* emtSub = "\xD0\x94\xD0\xBE\xD0\xB1\xD0\xB0\xD0\xB2\xD1\x8C\xD1\x82\xD0\xB5 \xD1\x81\xD1\x82\xD0\xB0\xD1\x82\xD1\x8C\xD0\xB8 \xD0\xB2 \xD0\xBB\xD0\xB0\xD1\x83\xD0\xBD\xD1\x87\xD0\xB5\xD1\x80\xD0\xB5"; 
        ImVec2 t2 = fontSegoeBold14->CalcTextSizeA(14.0f, FLT_MAX, 0.0f, emtSub);
        dl->AddText(fontSegoeBold14, 14.0f, ImVec2(icx - t2.x/2, icy+115), IM_COL32(92,99,112,255), emtSub);
        return;
    }
    ...
    for (int i = 0; i < (int)fineItems.size(); i++) {
        ...
        if (!fi.type.empty()) {
            bool isUK = (fi.type.find("\xD0\xA3\xD0\x9A") != std::string::npos); // "УК"
        ...
        if (fi.hasLicRevoke) {
            ...
            const char* bTxt = "\xD0\x92\xD0\xA3"; // Р вЂ™Р Р€ uppercase
    ...
    // "Р В Р’ВР В РЎС›Р В РЎвЂєР В РІР‚СљР В РЎвЂєР В РІР‚в„ўР В Р’В«Р В РІвЂћСћ Р В Р РѓР В РЎС›Р В Р’В Р В РЎвЂ™Р В Р’В¤" - SVG: x=20 y=25 font-size=12
    ...
    if (sy == 0) {
        ...
        const char* emClick1 = "\xD0\xA1\xD0\xBF\xD0\xB8\xD1\x81\xD0\xBE\xD0\xBA \xD0\xB2\xD1\x8B\xD0\xB4\xD0\xB0\xD1\x87\xD0\xB8 \xD1\x88\xD1\x82\xD1\x80\xD0\xB0\xD1\x84\xD0\xB0 \xD0\xBF\xD1\x83\xD1\x81\xD1\x82."; // Р РЋР С—Р С‘РЎРѓР С•Р С” Р Р†РЎвЂ№Р Т‘Р В°РЎвЂЎР С‘ РЎв‚¬РЎвЂљРЎР‚Р В°РЎвЂћР В° Р С—РЎС“РЎРѓРЎвЂљ.
        const char* emClick2 = "\xD0\x9A\xD0\xBB\xD0\xB8\xD0\xBA\xD0\xBD\xD0\xB8\xD1\x82\xD0\xB5 \xD0\xBF\xD0\xBE \xD1\x81\xD1\x82\xD0\xB0\xD1\x82\xD1\x8C\xD1\x8F\xD0\xBC \xD1\x81\xD0\xBB\xD0\xB5\xD0\xB2\xD0\xB0,"; // Р С™Р В»Р С‘Р С”Р Р…Р С‘РЎвЂљР Вµ Р С—Р С• РЎРѓРЎвЂљР В°РЎвЂљРЎРЉРЎРЏР С РЎРѓР В»Р ВµР Р†Р В°,
        const char* emClick3 = "\xD1\x87\xD1\x82\xD0\xBE\xD0\xB1\xD1\x8B \xD0\xB4\xD0\xBE\xD0\xB1\xD0\xB0\xD0\xB2\xD0\xB8\xD1\x82\xD1\x8C \xD1\x88\xD1\x82\xD1\x80\xD0\xB0\xD1\x84."; // Р РЋРІР‚РЋР РЋРІР‚С™Р В РЎвЂўР В Р’В±Р РЋРІР‚в„– Р В РўвЂР В РЎвЂўР В Р’В±Р В Р’В°Р В Р вЂ Р В РЎвЂР РЋРІР‚С™Р РЋР Р‰ Р РЋРІвЂљВ¬Р РЋРІР‚С™Р РЋР вЂљР В Р’В°Р РЋРІР‚С›.
    ...
    // "Р В РЎвЂєР В РІР‚ВР В Р’В©Р В РЎвЂ™Р В Р вЂЎ Р В Р Р‹Р В Р в‚¬Р В РЎС™Р В РЎС™Р В РЎвЂ™:" and total on the same visual line
    ...
    // "Р В Р Р‹ Р В Р’ВР В РІР‚вЂќР В Р вЂћР В Р вЂЎР В РЎС›Р В Р’ВР В РІР‚СћР В РЎС™ Р В РІР‚в„ўР В РЎвЂєР В РІР‚Сњ. Р В Р в‚¬Р В РІР‚СњР В РЎвЂєР В Р Р‹Р В РЎС›Р В РЎвЂєР В РІР‚в„ўР В РІР‚СћР В Р’В Р В РІР‚СћР В РЎСљР В Р’ВР В Р вЂЎ"
    float rvY = py + 185;
    ...
    // Buttons at bottom
    float btnY = py + 295;
    // "Р В РЎвЂєР В Р’В§Р В Р’ВР В Р Р‹Р В РЎС›Р В Р’ВР В РЎС›Р В Р’В¬" (clear) w=60 h=34
    ...
    // "Р В РІР‚в„ўР В Р’В«Р В РЎСџР В Р’ВР В Р Р‹Р В РЎвЂ™Р В РЎС›Р В Р’В¬ Р В Р РѓР В РЎС›Р В Р’В Р В РЎвЂ™Р В Р’В¤" (issue fine) w=105 h=34
    ...
    if (issClicked) {
        ...
        if (idStr.empty()) {
            ShowError("\xD0\x92\xD0\xB2\xD0\xB5\xD0\xB4\xD0\xB8\xD1\x82\xD0\xB5 ID \xD0\xBD\xD0\xB0\xD1\x80\xD1\x83\xD1\x88\xD0\xB8\xD1\x82\xD0\xB5\xD0\xBB\xD1\x8F!", C_RED); // Р вЂ™Р Р†Р ВµР Т‘Р С‘РЎвЂљР Вµ ID Р Р…Р В°РЎР‚РЎС“РЎв‚¬Р С‘РЎвЂљР ВµР В»РЎРЏ!
        } else {
            ...
            if (!hasSelected) {
                ShowError("\xD0\x92\xD1\x8B\xD0\xB1\xD0\xB5\xD1\x80\xD0\xB8\xD1\x82\xD0\xB5 \xD1\x85\xD0\xBE\xD1\x82\xD1\x8F \xD0\xB1\xD1\x8B \xD0\xBE\xD0\xB4\xD0\xBD\xD0\xBE \xD0\xBD\xD0\xB0\xD1\x80\xD1\x83\xD1\x88\xD0\xB5\xD0\xBD\xD0\xB8\xD0\xB5!", C_RED); // Р вЂ™РЎвЂ№Р В±Р ВµРЎР‚Р С‘РЎвЂљР Вµ РЎвЂ¦Р С•РЎвЂљРЎРЏ Р В±РЎвЂ№ Р С•Р Т‘Р Р…Р С• Р Р…Р В°РЎР‚РЎС“РЎв‚¬Р ВµР Р…Р С‘Р Вµ!
            } else {
...
void Gui::RenderWantedTab(ImDrawList* dl, ImVec2 o) {
    ...
    if (wantedItems.empty()) {
        ...
        pts[10] = pts[0];
        dl->AddPolyline(pts, 11, col, 0, 3.0f);

        const char* emtTxt = "\xD0\x91\xD0\x90\xD0\x97\xD0\x90 \xD0\xA0\xD0\x9E\xD0\x97\xD0\xAB\xD0\xA1\xD0\x9A\xD0\x90 \xD0\x9F\xD0\xA3\xD0\xA1\xD0\xA2\xD0\x90";
        ImVec2 t1 = fontSegoeBold20->CalcTextSizeA(20.0f, FLT_MAX, 0.0f, emtTxt);
        dl->AddText(fontSegoeBold20, 20.0f, ImVec2(icx - t1.x/2, icy+85), C_GRAY, emtTxt);
    ...
    if (sy == 0) {
        ...
        const char* emClick1 = "\xD0\xA1\xD0\xBF\xD0\xB8\xD1\x81\xD0\xBE\xD0\xBA \xD0\xBD\xD0\xB0\xD1\x80\xD1\x83\xD1\x88\xD0\xB5\xD0\xBD\xD0\xB8\xD0\xB9 \xD0\xBF\xD1\x83\xD1\x81\xD1\x82."; // РЎРїРёСЃРѕРє РЅР°СЂСѓС€РµРЅРёР№ РїСѓСЃС‚.
        ...
        const char* emClick3 = "\xD1\x87\xD1\x82\xD0\xBE\xD0\xB1\xD1\x8B \xD0\xBE\xD0\xB1\xD1\x8A\xD1\x8F\xD0\xB2\xD0\xB8\xD1\x82\xD1\x8C \xD0\xB2 \xD1\x80\xD0\xBE\xD0\xB7\xD1\x8B\xD1\x81\xD0\xBA."; // С‡С‚РѕР±С‹ РѕР±СЉСЏРІРёС‚СЊ РІ СЂРѕР·С‹СЃРє.
    ...
    // Buttons at bottom
    float btnY = py + 295;
    // "Р В РЎвЂєР В Р’В§Р В Р’ЛњР В Р Р‹Р В РЎС›Р В Р’ЛњР В РЎС›Р В Р’В¬" (clear) w=60 h=34
    ImGui::SetCursorScreenPos(ImVec2(px+20, btnY));
    ...
    // "Р В РІР‚в„ўР В Р’В«Р В РЎСџР В Р’ЛњР В Р Р‹Р В РЎвЂ™Р В РЎС›Р В Р’В¬ Р В Р РѓР В РЎС›Р В Р’В Р В РЎвЂ™Р В Р’В¤" (issue wanted) w=105 h=34
    ImGui::SetCursorScreenPos(ImVec2(px+85, btnY));
        ...
        if (idStr.empty()) {
            ...
            if (!hasSelected) {
                ...
                if (doQuote) {
                    ...
                    quoteStr += "\xD0\xA1\xD1\x83\xD0\xBC\xD0\xBC\xD0\xBC\xD0\xB0\xD1\x80\xD0\xBD\xD0\xBE " + totAmt + " \xD0\xBB\xD0\xB8\xD1\x88\xD0\xB5\xD0\xBD\xD0\xB8\xD1\x8F \xD1\x81\xD0\xB2\xD0\xBE\xD0\xB1\xD0\xBE\xD0\xB4\xD1\x8B.";
                }
...
void Gui::RenderBinderTab(ImDrawList* dl, ImVec2 o) {
    ...
    int visibleBindsCount = 0;
    for (auto& b : binds) if (b.name != "Radial Menu") visibleBindsCount++;
    
    // Empty state
    if (visibleBindsCount == 0) {
        float icx = o.x + 350, icy = o.y + 210; float cx = icx, cy = icy;
        ImU32 col = C_GRAY;
        dl->AddRect(ImVec2(cx-50, cy-30), ImVec2(cx+50, cy+30), col, 3.0f, 0, 3.0f);
        dl->AddRectFilled(ImVec2(cx-36, cy-16), ImVec2(cx-20, cy-8), col);
        dl->AddRectFilled(ImVec2(cx-12, cy-16), ImVec2(cx+12, cy-8), col);
        dl->AddRectFilled(ImVec2(cx+20, cy-16), ImVec2(cx+36, cy-8), col);
        dl->AddRectFilled(ImVec2(cx-36, cy+4), ImVec2(cx-20, cy+12), col);
        dl->AddRectFilled(ImVec2(cx-12, cy+4), ImVec2(cx+36, cy+12), col);

        const char* emtTxt = "\xD0\x91\xD0\x98\xD0\x9D\xD0\x94\xD0\xAB \xD0\x9D\xD0\x95 \xD0\x9D\xD0\x90\xD0\xA1\xD0\xA2\xD0\xA0\xD0\x9E\xD0\x95\xD0\x9D\xD0\xAB";
        ImVec2 t1 = fontSegoeBold20->CalcTextSizeA(20.0f, FLT_MAX, 0.0f, emtTxt);
        dl->AddText(fontSegoeBold20, 20.0f, ImVec2(icx - t1.x/2, icy+85), C_GRAY, emtTxt);

        const char* emtSub = "\xD0\xA1\xD0\xBE\xD0\xB7\xD0\xB4\xD0\xB0\xD0\xB9\xD1\x82\xD0\xB5 \xD0\xBF\xD1\x80\xD0\xBE\xD1\x84\xD0\xB8\xD0\xBB\xD1\x8C \xD0\xB8 \xD0\xB4\xD0\xBE\xD0\xB1\xD0\xB0\xD0\xB2\xD1\x8C\xD1\x82\xD0\xB5 \xD0\xB1\xD0\xB8\xD0\xBD\xD0\xB4\xD1\x8B \xD0\xB2 \xD0\xBB\xD0\xB0\xD1\x83\xD0\xBD\xD1\x87\xD0\xB5\xD1\x80\xD0\xB5."; 
        ImVec2 t2 = fontSegoeBold14->CalcTextSizeA(14.0f, FLT_MAX, 0.0f, emtSub);
        dl->AddText(fontSegoeBold14, 14.0f, ImVec2(icx - t2.x/2, icy+115), IM_COL32(92,99,112,255), emtSub);
        return;
    }

    // Left sidebar - only drawn when binds exist
    ...
    // "Р В РІР‚СљР В Р’В Р В Р в‚¬Р В РЎСџР В РЎСџР В Р’В« Р В РІР‚ЛњР В Р’ЛњР В РЎСљР В РІР‚СњР В РЎвЂєР В РІР‚в„ў" - SVG: font-size=10 font-weight=900 letter-spacing=1
    dl->AddText(fontSegoeBold14, 12.0f, ImVec2(sx+12, sy+12), C_GRAY,
        "\xD0\x93\xD0\xa0\xD0\xa3\xD0\x9F\xD0\x9F\xD0\xAB \xD0\x91\xD0\x98\xD0\x9D\xD0\x94\xD0\x9E\xD0\x92");
    dl->AddLine(ImVec2(sx+10, sy+35), ImVec2(sx+130, sy+35), C_BORDER, 1.5f);
    ...
    for (auto& b : binds) {
        ...
        if (!b.Group.empty()) {
            ...
            if (lGroup == "\xd0\xb2\xd1\x81\xd0\xb5" || lGroup == "all") continue; 
    ...
    // Sidebar: "Р’СЃРµ Р±РёРЅРґС‹ (N)" - SVG: x=10 y=55 w=120 h=26
    float gy = sy + 45;
    bool allSel = (selectedBindGroup == -1);
    char allLabel[64];
    sprintf_s(allLabel, "\xD0\x92\xD1\x81\xD0\xB5 \xD0\xB1\xD0\xB8\xD0\xBD\xD0\xB4\xD1\x8B (%d)", visibleBindsCount);
    if (allSel) {
    ...
    float rw = 514; // changed from 494 so that rx+rw = o.x+180+514 = o.x+694 (matches Settings right edge exactly)
    ...
    // Cards area - SVG: cards 235wР вЂњРІР‚вЂќ45h, two columns, 10px gap, starting at y=45 from search
    float cardsY = ry + 45;
    float cardW = 245;
    float cardH = 45;
    ...
    for (int i = 0; i < (int)binds.size(); i++) {
        ...
        // Smooth triangle (play arrow) Р Р†Р вЂљРІР‚Сњ compact, with 3px padding from circle edge
        ImVec2 triPts[3] = {
    ...
    ImGui::SetCursorScreenPos(ImVec2(bp.x, bp.y + by));
...
void Gui::RenderSettingsTab(ImDrawList* parent_dl, ImVec2 origin) {
    ...
    auto FadeCol = [](ImU32 col) -> ImU32 {
        float a_mult = ImGui::GetStyle().Alpha;
        if (a_mult >= 0.99f) return col;
        a_mult = 0.30f; // Stronger fade!
        int r = (col & 0xFF);
        int g = ((col >> 8) & 0xFF);
        int b = ((col >> 16) & 0xFF);
        int a = ((col >> 24) & 0xFF);
    ...
    auto drawToggle = [&](float ty, float tx, const char* title, const char* desc, bool& val, const char* id) {
        dl->AddText(Gui::fontSegoeBold14, 14.0f, ImVec2(tx+20, ty+6), FadeCol(C_WHITE), title);
        dl->AddText(Gui::fontSegoeBold12, 11.0f, ImVec2(tx+20, ty+24), FadeCol(IM_COL32(107,115,127,255)), desc);
        ImU32 rc = val ? IM_COL32(46,160,67,255) : C_BORDER;
        dl->AddRectFilled(ImVec2(tx+269, ty+5), ImVec2(tx+305, ty+25), FadeCol(rc), 10.0f);
        if (!val) dl->AddRect(ImVec2(tx+269, ty+5), ImVec2(tx+305, ty+25), FadeCol(C_LINE), 10.0f, 0, 1.0f);
        dl->AddCircleFilled(ImVec2(tx + (val ? 295 : 279), ty+15), 7.0f, FadeCol(val ? C_WHITE : C_GRAY), 16);
        ImGui::SetCursorScreenPos(ImVec2(tx+269, ty+5));
        if (ImGui::InvisibleButton(id, ImVec2(36, 20))) { val = !val; SaveSettings(); }
    };

    // ----- LEFT COLUMN -----
    // Block 1: VISUAL SETTINGS
    dl->AddRectFilled(ImVec2(sx, sy), ImVec2(sx+325, sy+210), C_HEADER, 8.0f);
    dl->AddRect(ImVec2(sx, sy), ImVec2(sx+325, sy+210), C_BORDER, 8.0f, 0, 1.5f);
    ...
    dl->AddLine(ImVec2(sx+20, sy+35), ImVec2(sx+305, sy+35), C_BORDER, 1.5f);
    ...
    dl->AddText(fontSegoeBold20, 16.0f, ImVec2(sx+305-ops.x, ty2+4), C_GOLD, opBuf);
    float slY = ty2+30;
    dl->AddRectFilled(ImVec2(sx+20, slY), ImVec2(sx+305, slY+6), C_BORDER, 3.0f);
    dl->AddRectFilled(ImVec2(sx+20, slY), ImVec2(sx+20+(settingsAlpha*285.0f), slY+6), C_GOLD, 3.0f);
    dl->AddCircleFilled(ImVec2(sx+20+(settingsAlpha*285.0f), slY+3), 7.0f, C_BOX, 16);
    dl->AddCircle(ImVec2(sx+20+(settingsAlpha*285.0f), slY+3), 7.0f, C_GOLD, 16, 2.0f);
    ImGui::SetCursorScreenPos(ImVec2(sx+20, slY-6));
    ImGui::InvisibleButton("##alphaSlider", ImVec2(285, 18));
    if (ImGui::IsItemActive()) {
        settingsAlpha = (ImGui::GetMousePos().x - (sx+20)) / 285.0f;
    ...
    dl->AddLine(ImVec2(sx+20, tabY+95), ImVec2(sx+305, tabY+95), C_BORDER, 1.0f);
    ...
    dl->AddLine(ImVec2(sx+20, tabY+150), ImVec2(sx+305, tabY+150), C_BORDER, 1.0f);
    ...
    float ry0 = sy;
    dl->AddRectFilled(ImVec2(prx, ry0), ImVec2(prx+325, ry0+515), C_HEADER, 8.0f);
    dl->AddRect(ImVec2(prx, ry0), ImVec2(prx+325, ry0+515), C_BORDER, 8.0f, 0, 1.5f);
    ...
    dl->AddLine(ImVec2(prx+20, ry0+35), ImVec2(prx+305, ry0+35), C_BORDER, 1.5f);
    ...
    float boxH = 22.0f;
    float boxY = ry1 + 2.0f;
    dl->AddRectFilled(ImVec2(prx+305-boxW, boxY), ImVec2(prx+305, boxY+boxH), (C_GOLD & 0x00FFFFFF) | (25 << 24), 4.0f);
    dl->AddRect(ImVec2(prx+305-boxW, boxY), ImVec2(prx+305, boxY+boxH), C_GOLD, 4.0f, 0, 1.5f);
    dl->AddText(fontSegoeBold14, 14.0f, ImVec2(prx+305-boxW/2-ks.x/2, boxY+boxH/2-ks.y/2), C_GOLD, keyStr.c_str());
    
    dl->AddLine(ImVec2(prx+20, ry1+45), ImVec2(prx+305, ry1+45), C_BORDER, 1.0f);
    // Delay
    float ry2 = ry1 + 55;
    ...
    dl->AddText(fontSegoeBold20, 16.0f, ImVec2(prx+305-dss.x, ry2+1), C_GOLD, delBuf);
    float sdlY = ry2+25;
    float pct = (float)(binderDelay - 250) / 2750.0f;
    if (pct < 0.0f) pct = 0.0f; if (pct > 1.0f) pct = 1.0f;
    dl->AddRectFilled(ImVec2(prx+20, sdlY), ImVec2(prx+305, sdlY+6), C_BORDER, 3.0f);
    dl->AddRectFilled(ImVec2(prx+20, sdlY), ImVec2(prx+20+(pct*285.0f), sdlY+6), C_GOLD, 3.0f);
    dl->AddCircleFilled(ImVec2(prx+20+(pct*285.0f), sdlY+3), 7.0f, C_BOX, 16);
    dl->AddCircle(ImVec2(prx+20+(pct*285.0f), sdlY+3), 7.0f, C_GOLD, 16, 2.0f);
    ImGui::SetCursorScreenPos(ImVec2(prx+20, sdlY-6));
    ImGui::InvisibleButton("##delaySlider", ImVec2(285, 18));
    if (ImGui::IsItemActive()) {
        float activePct = (ImGui::GetMousePos().x - (prx+20)) / 285.0f;
        if (activePct < 0.0f) activePct = 0.0f; if (activePct > 1.0f) activePct = 1.0f;
        binderDelay = 250 + (int)round(activePct * 11.0f) * 250;
    }
    if (ImGui::IsItemDeactivated()) SaveSettings();

    dl->AddLine(ImVec2(prx+20, ry2+45), ImVec2(prx+305, ry2+45), C_BORDER, 1.0f);
    ...
    dl->AddLine(ImVec2(prx+20, ry2+95), ImVec2(prx+305, ry2+95), C_BORDER, 1.0f);
    ...
    dl->AddLine(ImVec2(prx+20, ry2+145), ImVec2(prx+305, ry2+145), C_BORDER, 1.0f);
    drawToggle(ry2+150, prx, "\xD0\x9F\xD0\xBE\xD0\xB8\xD1\x81\xD0\xBA \xD0\xB2 \xD1\x82\xD0\xB5\xD0\xBA\xD1\x83\xD1\x89\xD0\xB5\xD0\xBC \xD1\x80\xD0\xB0\xD0\xB7\xD0\xB4\xD0\xB5\xD0\xBB\xD0\xB5", "\xD0\x98\xD1\x81\xD0\xBA\xD0\xB0\xD1\x82\xD1\x8C \xD1\x82\xD0\xBE\xD0\xBB\xD1\x8C\xD0\xBA\xD0\xBE \xD0\xB2 \xD0\xBE\xD1\x82\xD0\xBA\xD1\x80\xD1\x8B\xD1\x82\xD0\xBE\xD0\xBC \xD1\x80\xD0\xB0\xD0\xB7\xD0\xB4\xD0\xB5\xD0\xBB\xD0\xB5 \xD0\xB7\xD0\xB0\xD0\xBA\xD0\xBE\xD0\xBD\xD0\xBE\xD0\xB2", searchCurrentSection, "##schCurrSecBtn");
    dl->AddLine(ImVec2(prx+20, ry2+195), ImVec2(prx+305, ry2+195), C_BORDER, 1.0f);
    drawToggle(ry2+200, prx, "\xD0\x9A\xD0\xBB\xD0\xB8\xD0\xBA-\xD1\x80\xD0\xB5\xD0\xB6\xD0\xB8\xD0\xBC \xD0\xBA\xD1\x80\xD1\x83\xD0\xB3. \xD0\xBC\xD0\xB5\xD0\xBD\xD1\x8E", "\xD0\x90\xD0\xBA\xD1\x82\xD0\xB8\xD0\xB2\xD0\xB0\xD1\x86\xD0\xB8\xD1\x8F \xD0\xBA\xD1\x80\xD1\x83\xD0\xB3\xD0\xBE\xD0\xB2\xD0\xBE\xD0\xB3\xD0\xBE \xD0\xBC\xD0\xB5\xD0\xBD\xD1\x8E \xD0\xBF\xD0\xBE \xD0\xBA\xD0\xBB\xD0\xB8\xD0\xBA\xD1\x83 \xD0\xB2\xD0\xBC\xD0\xB5\xD1\x81\xD1\x82\xD0\xBE \xD1\x83\xD0\xB4\xD0\xB5\xD1\x80\xD0\xB6\xD0\xB0\xD0\xBD\xD0\xB8\xD1\x8F.", radialActivationToggleMode, "##radToggleBtn");
    dl->AddLine(ImVec2(prx+20, ry2+245), ImVec2(prx+305, ry2+245), C_BORDER, 1.0f);
    drawToggle(ry2+250, prx, "\xD0\x9D\xD0\xB0\xD0\xBF\xD0\xBE\xD0\xBC\xD0\xB8\xD0\xBD\xD0\xB0\xD0\xBD\xD0\xB8\xD0\xB5 \xD0\xBF\xD0\xB5\xD1\x80\xD0\xB5\xD0\xB4 PAYDAY", "\xD0\xA3\xD0\xB2\xD0\xB5\xD0\xB4\xD0\xBE\xD0\xBC\xD0\xBB\xD0\xB5\xD0\xBD\xD0\xB8\xD0\xB5 \xD0\xB7\xD0\xB0 10 \xD0\xBC\xD0\xB8\xD0\xBD \xD0\xB8 30 \xD1\x81\xD0\xB5\xD0\xBA \xD0\xBF\xD0\xB5\xD1\x80\xD0\xB5\xD0\xB4 PAYDAY.", notifyPayday, "##tglN2");
    dl->AddLine(ImVec2(prx+20, ry2+295), ImVec2(prx+305, ry2+295), C_BORDER, 1.0f);
    drawToggle(ry2+300, prx, "\xD0\x9E\xD1\x87\xD0\xB8\xD1\x81\xD1\x82\xD0\xBA\xD0\xB0 \xD0\xBF\xD0\xBE\xD0\xB8\xD1\x81\xD0\xBA\xD0\xB0 \xD0\xBF\xD1\x80\xD0\xB8 \xD0\xB7\xD0\xB0\xD0\xBA\xD1\x80\xD1\x8B\xD1\x82\xD0\xB8\xD0\xB8", "\xD0\xA1\xD0\xB1\xD1\x80\xD0\xB0\xD1\x81\xD1\x8B\xD0\xB2\xD0\xB0\xD1\x82\xD1\x8C \xD1\x82\xD0\xB5\xD0\xBA\xD1\x81\xD1\x82 \xD0\xB2 \xD1\x81\xD1\x82\xD1\x80\xD0\xBE\xD0\xBA\xD0\xB5 \xD0\xBF\xD0\xBE\xD0\xB8\xD1\x81\xD0\xBA\xD0\xB0 \xD0\xBF\xD1\x80\xD0\xB8 \xD0\xB7\xD0\xB0\xD0\xBA\xD1\x80\xD1\x8B\xD1\x82\xD0\xB8\xD0\xB8 \xD0\xBE\xD0\xB2\xD0\xB5\xD1\x80\xD0\xBB\xD0\xB5\xD1\x8F", clearSearchOnClose, "##tglCSOC");
    dl->AddLine(ImVec2(prx+20, ry2+345), ImVec2(prx+305, ry2+345), C_BORDER, 1.0f);
    drawToggle(ry2+350, prx, "\xD0\x97\xD0\xB0\xD0\xBA\xD1\x80\xD1\x8B\xD0\xB2\xD0\xB0\xD1\x82\xD1\x8C \xD0\xBF\xD0\xBE \xD0\xBA\xD0\xBB\xD0\xB8\xD0\xBA\xD1\x83 \xD0\xB2\xD0\xBD\xD0\xB5 \xD0\xBE\xD0\xBA\xD0\xBD\xD0\xB0", "\xD0\x97\xD0\xB0\xD0\xBA\xD1\x80\xD1\x8B\xD0\xB2\xD0\xB0\xD1\x82\xD1\x8C \xD0\xBE\xD0\xB2\xD0\xB5\xD1\x80\xD0\xBB\xD0\xB5\xD0\xB9 \xD0\xBF\xD1\x80\xD0\xB8 \xD0\xBA\xD0\xBB\xD0\xB8\xD0\xBA\xD0\xB5 \xD0\xBC\xD1\x8B\xD1\x88\xD0\xBA\xD0\xBE\xD0\xB9 \xD0\xBC\xD0\xB8\xD0\xBC\xD0\xBE \xD0\xBE\xD0\xBA\xD0\xBD\xD0\xB0", closeOnClickOutside, "##tglCOCO");


    // Block 4: SMART QUOTING & NOTIFICATIONS
    float qy = sy + 525;
    dl->AddRectFilled(ImVec2(prx, qy), ImVec2(prx+325, qy+370), C_HEADER, 8.0f);
    dl->AddRect(ImVec2(prx, qy), ImVec2(prx+325, qy+370), C_BORDER, 8.0f, 0, 1.5f);
    ...
    dl->AddLine(ImVec2(prx+20, qy+35), ImVec2(prx+305, qy+35), C_BORDER, 1.5f);

    drawToggle(qy+45, prx, "\xD0\x92\xD0\xBA\xD0\xBB\xD1\x8E\xD1\x87\xD0\xB8\xD1\x82\xD1\x8C \xD1\x86\xD0\xB8\xD1\x82\xD0\xB8\xD1\x80\xD0\xBE\xD0\xB2\xD0\xB0\xD0\xBD\xD0\xB8\xD0\xB5 \xD1\x82\xD0\xB5\xD0\xBA\xD1\x81\xD1\x82\xD0\xB0 \xD0\xB7\xD0\xB0\xD0\xBA\xD0\xBE\xD0\xBD\xD0\xB0", "\xD0\x97\xD0\xB0\xD1\x87\xD0\xB8\xD1\x82\xD1\x8B\xD0\xB2\xD0\xB0\xD1\x82\xD1\x8C \xD1\x82\xD0\xB5\xD0\xBA\xD1\x81\xD1\x82 \xD1\x81\xD1\x82\xD0\xB0\xD1\x82\xD1\x8C\xD0\xB8 \xD0\xBF\xD0\xBE \xD0\xBA\xD0\xBB\xD0\xB8\xD0\xBA\xD1\x83 \xD0\xBD\xD0\xB0 \xD0\xBD\xD0\xB5\xD0\xB5.", quoteEnabled, "##tglQ1");
    
    if (!quoteEnabled) {
        ImGui::BeginDisabled();
    }
    
    dl->AddLine(ImVec2(prx+20, qy+95), ImVec2(prx+305, qy+95), C_BORDER, 1.0f);
    drawToggle(qy+100, prx, "\xD0\xA0\xD0\xB0\xD1\x81\xD1\x88\xD0\xB8\xD1\x80\xD0\xB5\xD0\xBD\xD0\xBD\xD0\xBE\xD0\xB5 \xD1\x86\xD0\xB8\xD1\x82\xD0\xB8\xD1\x80\xD0\xBE\xD0\xB2\xD0\xB0\xD0\xBD\xD0\xB8\xD0\xB5 \xD1\x82\xD0\xB5\xD0\xBA\xD1\x81\xD1\x82\xD0\xB0 \xD0\xB7\xD0\xB0\xD0\xBA\xD0\xBE\xD0\xBD\xD0\xB0", "\xD0\x94\xD0\xBE\xD0\xB1\xD0\xB0\xD0\xB2\xD0\xBB\xD1\x8F\xD1\x82\xD1\x8C \xD0\xBD\xD0\xB0\xD0\xB7\xD0\xB2\xD0\xB0\xD0\xBD\xD0\xB8\xD0\xB5, \xD1\x80\xD0\xB0\xD0\xB7\xD0\xB4\xD0\xB5\xD0\xBB, \xD0\xBD\xD0\xBE\xD0\xBC\xD0\xB5\xD1\x80 \xD1\x81\xD1\x82\xD0\xB0\xD1\x82\xD1\x8C\xD0\xB8 \xD0\xB2 \xD1\x86\xD0\xB8\xD1\x82\xD0\xB8\xD1\x80\xD0\xBE\xD0\xB2\xD0\xB0\xD0\xBD\xD0\xB8\xD0\xB5.", quoteExtended, "##tglQ2");
    dl->AddLine(ImVec2(prx+20, qy+150), ImVec2(prx+305, qy+150), C_BORDER, 1.0f);
    ...
    dl->AddLine(ImVec2(prx+20, qy+205), ImVec2(prx+305, qy+205), C_BORDER, 1.0f);
    drawToggle(qy+210, prx, "\xD0\xA6\xD0\xB8\xD1\x82\xD0\xB8\xD1\x80\xD0\xBE\xD0\xB2\xD0\xB0\xD0\xBD\xD0\xB8\xD0\xB5 \xD0\xB2 \xD0\xB1\xD0\xBB\xD0\xBE\xD0\xBA\xD0\xBD\xD0\xBE\xD1\x82\xD0\xB5", "\xD0\x92\xD1\x8B\xD0\xB4\xD0\xB5\xD0\xBB\xD0\xB8\xD1\x82\xD0\xB5 \xD1\x82\xD0\xB5\xD0\xBA\xD1\x81\xD1\x82 \xD0\xB8 \xD0\xBD\xD0\xB0\xD0\xB6\xD0\xBC\xD0\xB8\xD1\x82\xD0\xB5 \xD0\x9F\xD0\x9A\xD0\x9C \xD1\x87\xD1\x82\xD0\xBE\xD0\xB1\xD1\x8B \xD0\xB7\xD0\xB0\xD1\x86\xD0\xB8\xD1\x82\xD0\xB8\xD1\x80\xD0\xBE\xD0\xB2\xD0\xB0\xD1\x82\xD1\x8C.", quoteNotepad, "##tglQ3_5");
    
    if (!quoteEnabled) {
        ImGui::EndDisabled();
    }
    
    dl->AddLine(ImVec2(prx+20, qy+260), ImVec2(prx+305, qy+260), C_BORDER, 1.0f);
    drawToggle(qy+265, prx, "\xD0\xA6\xD0\xB8\xD1\x82\xD0\xB8\xD1\x80\xD0\xBE\xD0\xB2\xD0\xB0\xD0\xBD\xD0\xB8\xD0\xB5 \xD1\x88\xD1\x82\xD1\x80\xD0\xB0\xD1\x84\xD0\xBE\xD0\xB2", "\xD0\xA0\xD0\xB5\xD0\xB6\xD0\xB8\xD0\xBC \xD1\x86\xD0\xB8\xD1\x82\xD0\xB8\xD1\x80\xD0\xBE\xD0\xB2\xD0\xB0\xD0\xBD\xD0\xB8\xD1\x8F \xD1\x88\xD1\x82\xD1\x80\xD0\xB0\xD1\x84\xD0\xBE\xD0\xB2 \xD0\xBF\xD0\xBE\xD1\x81\xD0\xBB\xD0\xB5 \xD0\xBE\xD1\x84\xD0\xBE\xD1\x80\xD0\xBC\xD0\xBB\xD0\xB5\xD0\xBD\xD0\xB8\xD1\x8F.", quoteFines, "##tglQ4");
    dl->AddLine(ImVec2(prx+20, qy+315), ImVec2(prx+305, qy+315), C_BORDER, 1.0f);
    ...
    dl->AddRect(ImVec2(sx, vkY), ImVec2(sx+324, vkY+vkH), IM_COL32(0, 119, 255, 255), 8.0f, 0, 1.5f);
    
    // VK Logo rounded rect
    float logoX = sx + 25, logoY = vkY + vkH/2 - 50; // Centered vertically
    ...
    // Titles
    dl->AddText(fontSegoeBold12, 12.0f, ImVec2(sx+75, logoY+2), C_WHITE, "\xD0\x9E\xD0\xA4\xD0\x98\xD0\xA6\xD0\x98\xD0\x90\xD0\x9B\xD0\xAC\xD0\x9D\xD0\x9E\xD0\x95 \xD0\xA1\xD0\x9E\xD0\x9E\xD0\x91\xD0\xA9\xD0\x95\xD0\xA1\xD0\xA2\xD0\x92\xD0\x9E");
    dl->AddText(fontArialBlack24, 16.0f, ImVec2(sx+75, logoY+16), IM_COL32(0, 119, 255, 255), "DURAN HELPER");
    
    // Small line
    dl->AddLine(ImVec2(sx+25, logoY+55), ImVec2(sx+295, logoY+55), C_BORDER, 1.0f);
    ...
    dl->AddText(fontSegoeBold12, 12.0f, ImVec2(sx + 160 - sdSize.x/2, logoY + 70), C_GRAY, "\xD0\x90\xD0\xBA\xD1\x82\xD1\x83\xD0\xB0\xD0\xBB\xD1\x8C\xD0\xBD\xD1\x8B\xD0\xB5 \xD0\xB2\xD0\xB5\xD1\x80\xD1\x81\xD0\xB8\xD0\xB8 \xD0\xB8 \xD0\xBF\xD0\xBE\xD0\xB4\xD0\xB4\xD0\xB5\xD1\x80\xD0\xB6\xD0\xBA\xD0\xB0");
    
    // Button
    float btnW = 200, btnH = 36;
    float btnX = sx + 160 - btnW/2, btnY = logoY + 105;
    ...
    // Bottom separator
    float bY = qy + 385; // aligned to end of right block (qy + 370) + 15
    ...
    if (resClicked) {
        ...
        rememberTab = true; searchCurrentSection = true; quoteEnabled = true; quoteNotepad = true;
...
void Gui::RenderBinderHint() {
    ...
    if (panelW < 200.0f) panelW = 200.0f; // Min width for "DURAN HELPER" header
    ...
    // 4. Gold accent top-left вЂ” offset 1px outward so line sits above edge, no inner glow
    ...
    dl->AddText(fontArialBlack24, 20.0f, ImVec2(p0.x+15+duranW, p0.y+12), C_GOLD, "HELPER");
    float helperW = fontArialBlack24->CalcTextSizeA(20.0f, FLT_MAX, 0.0f, "HELPER").x;
    ...
    const char* subTitle = "\xD0\x90\xD0\x9A\xD0\xA2\xD0\xA3\xD0\x90\xD0\x9B\xD0\xAC\xD0\x9D\xD0\xAB\xD0\x95 \xD0\x91\xD0\x98\xD0\x9D\xD0\x94\xD0\xAB"; // РђРљРўРЈРђР›Р¬РќР«Р• Р‘РРќР”Р«
    ...
    if (activeCount == 0) {
        cy += 5.0f;
        const char* emptyTxt = "\xD0\x9D\xD0\xB5\xD1\x82 \xD0\xB0\xD0\xBA\xD1\x82\xD0\xB8\xD0\xB2\xD0\xBD\xD1\x8B\xD1\x85 \xD0\xB1\xD0\xB8\xD0\xBD\xD0\xB4\xD0\xBE\xD0\xB2"; // РќРµС‚ Р°РєС‚РёРІРЅС‹С… Р±РёРЅРґРѕРІ
...
void Gui::Render() {
    ...
    if (!show) {
        alpha = 0.0f; // Instant close Р Р†Р вЂљРІР‚Сњ no fade out
    } else {
    ...
    if (alpha >= 0.01f) {
        ...
        if (true) {
            if (activeTab == -1 && !showSettings) {
                DrawGridMenu(dl, origin);
            } else {
                DrawGridBackButton(dl, origin);
                if (showSettings) {
                    ...
                    switch (activeTab) {
                        ...
                        case 5: RenderPatrolsTab(dl, origin); break;
                    }
                }
            }
        } else {
        ...
        if (closeOnClickOutside && ImGui::IsMouseReleased(0)) {
            ImVec2 mp = ImGui::GetMousePos();
            if (mp.x < origin.x || mp.x > origin.x + W || mp.y < origin.y || mp.y > origin.y + H) {
                Toggle();
            }
        }
...
void Gui::RenderRadialMenu() {
    // Prevent GTA SA from processing mouse clicks while radial menu is open
    // Zero CMouseControllerState (raw mouse state)
    *(uint8_t*)0xB73418 = 0; // NewMouseState.lmb
    *(uint8_t*)0xB73404 = 0; // OldMouseState.lmb
    *(uint8_t*)0xB73419 = 0; // NewMouseState.rmb
    *(uint8_t*)0xB73405 = 0; // OldMouseState.rmb
    *(uint8_t*)0xB7341A = 0; // NewMouseState.mmb
    *(uint8_t*)0xB73406 = 0; // OldMouseState.mmb
    ...
    ImU32 themeBg = C_BG; // Match ID input box color
    ...
    auto drawDonutSector = [&](float a1, float a2, float rIn, float rOut, ImU32 col, ImU32 divCol, bool hovered, bool isThickDiv) {
        ...
        for (int s = 0; s < steps; s++) {
            ...
            if (hovered) {
                dl->AddConvexPolyFilled(quad, 4, (C_GOLD & 0x00FFFFFF) | (35 << 24));
            }
        }
        dl->Flags = oldFlags;

        // Grid pattern (same style as main overlay) вЂ” always C_GRID, no color change on hover
        {
            ...
            for (float x = startX; x <= maxX; x += gridStep) {
                DrawClippedGridLine(dl, ImVec2(x, minY), ImVec2(x, maxY), cx, cy, rIn + 1.0f, rOut - 1.0f, a1, a2, C_GRID);
            }
            
            // Horizontal lines
            float startY = floorf(minY / gridStep) * gridStep;
            for (float y = startY; y <= maxY; y += gridStep) {
                DrawClippedGridLine(dl, ImVec2(minX, y), ImVec2(maxX, y), cx, cy, rIn + 1.0f, rOut - 1.0f, a1, a2, C_GRID);
            }
        }

        // Divider borders on BOTH sides of each sector (always visible)
        ...
        // Outer arc border along the outer edge
        for (int s = 0; s < steps; s++) {
            dl->AddLine(outerPts[s], outerPts[s+1], divCol, 2.0f);
        }
        // Inner arc border along the inner edge
        for (int s = 0; s < steps; s++) {
            dl->AddLine(innerPts[s], innerPts[s+1], divCol, 1.5f);
        }
    };
    ...
    if (radialMode == "Grouped") {
        ...
        if (dist > innerR && dist <= groupR) {
            ...
            if (!radialActivationToggleMode) {
                radialSelectedGroup = hovG; // auto-select on hover (hold mode only)
            }
            ...
            // In toggle (click) mode, don't deselect group when cursor moves to center
            if (!radialActivationToggleMode) {
        ...
        // в•ђв•ђв•ђ INNER RING (Groups) в•ђв•ђв•ђ
        ...
        // в•ђв•ђв•ђ OUTER RING (Sectors) в•ђв•ђв•ђ
...
void Gui::RenderNotifications() {
    ...
    static time_t next10MinTrigger = 0;
    static time_t next30SecTrigger = 0;

    if (notifyPayday) {
        time_t now = time(nullptr);
        if (lastCheckTime == 0) lastCheckTime = now;

        if (next10MinTrigger == 0 || next30SecTrigger == 0) {
            struct tm t; localtime_s(&t, &now);
            t.tm_min = 50; t.tm_sec = 0;
            next10MinTrigger = mktime(&t);
            if (next10MinTrigger <= now) next10MinTrigger += 3600;
            
            t.tm_min = 59; t.tm_sec = 30;
            next30SecTrigger = mktime(&t);
            if (next30SecTrigger <= now) next30SecTrigger += 3600;
        }

        if (now > lastCheckTime) {
            bool wasMinimized = (now - lastCheckTime > 3);
    ...
    float startY = ds.y - 140.0f;
    if (activePatrol.active) startY -= 130.0f; // offset from bottom and patrol widget
...
void Gui::RenderPatrolWidget() {
	...
	// Position widget bottom-center (like notifications)
	ImVec2 screenSz = ImGui::GetIO().DisplaySize;
	static float pwX = -1.0f, pwY = -1.0f;
	float w = 380.0f, h = 120.0f, c = 12.0f;
	if (pwX < 0) { pwX = (screenSz.x - w) * 0.5f; pwY = screenSz.y - 140.0f; }

	// Draggable when overlay is open - needs proper window context to avoid "Debug" artifact
	if (show) {
		ImGui::SetNextWindowPos(ImVec2(pwX, pwY));
		ImGui::SetNextWindowSize(ImVec2(w, h));
		ImGui::PushStyleColor(ImGuiCol_WindowBg, 0);
		ImGui::PushStyleColor(ImGuiCol_Border, 0);
		ImGui::Begin("##PatrolWidgetDragArea", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoFocusOnAppearing);
		ImGui::InvisibleButton("##PatrolWidgetDrag", ImVec2(w, h));
		if (ImGui::IsItemActive()) {
			pwX += ImGui::GetIO().MouseDelta.x;
			pwY += ImGui::GetIO().MouseDelta.y;
		}
		ImGui::End();
		ImGui::PopStyleColor(2);
	}
	...
	// Octagon (same style as notification)
	ImVec2 poly[] = {
		{pwX, pwY+c}, {pwX+c, pwY}, {pwX+w-c, pwY}, {pwX+w, pwY+c},
		{pwX+w, pwY+h-c}, {pwX+w-c, pwY+h}, {pwX+c, pwY+h}, {pwX, pwY+h-c}
	};

	// Gold accent (drawn first, then bg covers inner half - same as notification)
	dl->AddLine(ImVec2(pwX-1.0f, pwY+c), ImVec2(pwX+c, pwY-1.0f), C_GOLD, 3.0f);
	dl->AddLine(ImVec2(pwX+c, pwY-1.0f), ImVec2(pwX+150.0f, pwY-1.0f), C_GOLD, 3.0f);
	...
	// Grid (clipped like notification)
	for (float x = 0; x < w; x += 30) {
		float sy2 = 0, ey2 = h;
		if (x < c) { sy2 = c - x; ey2 = h - (c - x); }
		else if (x > w - c) { sy2 = x - (w - c); ey2 = h - (x - (w - c)); }
		dl->AddLine(ImVec2(pwX+x, pwY+sy2), ImVec2(pwX+x, pwY+ey2), C_GRID, 1.0f);
	}
	for (float y = 0; y < h; y += 30) {
		float sx2 = 0, ex2 = w;
		if (y < c) { sx2 = c - y; ex2 = w - (c - y); }
		else if (y > h - c) { sx2 = y - (h - c); ex2 = w - (y - (h - c)); }
		dl->AddLine(ImVec2(pwX+sx2, pwY+y), ImVec2(pwX+ex2, pwY+y), C_GRID, 1.0f);
	}

	// Header block (same as notification: height 34)
	float HEADER_H = 34.0f;
	ImVec2 hdrPoly[] = {
		{pwX, pwY+c}, {pwX+c, pwY}, {pwX+w-c, pwY}, {pwX+w, pwY+c},
		{pwX+w, pwY+HEADER_H}, {pwX, pwY+HEADER_H}
	};
	dl->AddConvexPolyFilled(hdrPoly, 6, C_HEADER);
	dl->AddLine(ImVec2(pwX, pwY+HEADER_H), ImVec2(pwX+w, pwY+HEADER_H), C_BORDER, 2.0f);

	// Header text: DURAN (white) + PATROL (gold) - centered, same font as notification (fontArialBlack24 18px)
	std::string duranTxt = "DURAN";
	std::string patrolTxt = "HELPER";
	float duranW = fontArialBlack24->CalcTextSizeA(18.0f, FLT_MAX, 0.0f, duranTxt.c_str()).x;
	float patrolW = fontArialBlack24->CalcTextSizeA(18.0f, FLT_MAX, 0.0f, patrolTxt.c_str()).x;
	float totalHeaderW = duranW + 6.0f + patrolW;
	float headerX = pwX + (w - totalHeaderW) * 0.5f;
	float headerY = pwY + 8.0f;

	dl->AddText(fontArialBlack24, 18.0f, ImVec2(headerX, headerY), C_WHITE, duranTxt.c_str());
	dl->AddText(fontArialBlack24, 18.0f, ImVec2(headerX + duranW + 6.0f, headerY), C_GOLD, patrolTxt.c_str());

	// Version badge - dynamic, same style as notification (top-right)
	std::string ver = "V" + versionStr;
	float verFontSize = 11.0f;
	float verW = fontSegoeBold12->CalcTextSizeA(verFontSize, FLT_MAX, 0.0f, ver.c_str()).x + 10.0f;
	float badgeH = 15.0f;
	...
	if (activePatrol.lastTickTime == 0) activePatrol.lastTickTime = now;
	float dt = (float)(now - activePatrol.lastTickTime) / 1000.0f;
	activePatrol.lastTickTime = now;

	// Detect if game was minimized (dt > 3 seconds means frame gap)
	bool wasMinimized = (dt > 3.0f);
	float overdueSec = 0.0f;
	if (wasMinimized && activePatrol.timeRemainingSec > 0 && dt > activePatrol.timeRemainingSec) {
		overdueSec = dt - activePatrol.timeRemainingSec; // how late the report is
	}

	activePatrol.totalElapsedSec += dt;
	if (activePatrol.timeRemainingSec > 0) activePatrol.timeRemainingSec -= dt;

	// Auto-fire next stage
	if (activePatrol.timeRemainingSec <= 0) {
		if (activePatrol.totalMin > 0 && activePatrol.totalElapsedSec >= activePatrol.totalMin * 60.0f) {
			// End of patrol
			if (wasMinimized && overdueSec > 120.0f) {
				// Past 2min tolerance - skip final report
				RunOnMainThread([]() {
					extern void AddLocalSAMPMessage(const char*);
					AddLocalSAMPMessage(UTF8ToCP1251("{D2A65E}[DURAN HELPER] {FF6B6B}\xD0\x98\xD0\xB3\xD1\x80\xD0\xB0 \xD0\xB1\xD1\x8B\xD0\xBB\xD0\xB0 \xD1\x81\xD0\xB2\xD1\x91\xD1\x80\xD0\xBD\xD1\x83\xD1\x82\xD0\xB0, \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4 \xD0\xBF\xD1\x80\xD0\xBE\xD0\xBF\xD1\x83\xD1\x89\xD0\xB5\xD0\xBD. \xD0\x9F\xD0\xBE\xD0\xB3\xD1\x80\xD0\xB5\xD1\x88\xD0\xBD\xD0\xBE\xD1\x81\xD1\x82\xD1\x8C (2 \xD0\xBC\xD0\xB8\xD0\xBD) \xD0\xBF\xD1\x80\xD0\xB5\xD0\xB2\xD1\x8B\xD1\x88\xD0\xB5\xD0\xBD\xD0\xB0.").c_str());
				});
			} else {
				if (wasMinimized && overdueSec > 0) {
					RunOnMainThread([]() {
						extern void AddLocalSAMPMessage(const char*);
						AddLocalSAMPMessage(UTF8ToCP1251("{D2A65E}[DURAN HELPER] {FFFFFF}\xD0\x98\xD0\xB3\xD1\x80\xD0\xB0 \xD0\xB1\xD1\x8B\xD0\xBB\xD0\xB0 \xD1\x81\xD0\xB2\xD1\x91\xD1\x80\xD0\xBD\xD1\x83\xD1\x82\xD0\xB0, \xD0\xBD\xD0\xBE \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4 \xD0\xBE\xD1\x82\xD0\xBF\xD1\x80\xD0\xB0\xD0\xB2\xD0\xBB\xD0\xB5\xD0\xBD \xD0\xB2 \xD0\xBF\xD1\x80\xD0\xB5\xD0\xB4\xD0\xB5\xD0\xBB\xD0\xB0\xD1\x85 \xD0\xBF\xD0\xBE\xD0\xB3\xD1\x80\xD0\xB5\xD1\x88\xD0\xBD\xD0\xBE\xD1\x81\xD1\x82\xD0\xB8 (2 \xD0\xBC\xD0\xB8\xD0\xBD).").c_str());
					});
				}
				activePatrol.currentStage = 2; // END
				ExecutePatrolReport();
			}
			activePatrol.active = false;
		} else {
			if (wasMinimized && overdueSec > 120.0f) {
				// Past 2min tolerance - skip report, schedule next
				RunOnMainThread([]() {
					extern void AddLocalSAMPMessage(const char*);
					AddLocalSAMPMessage(UTF8ToCP1251("{D2A65E}[DURAN HELPER] {FF6B6B}\xD0\x98\xD0\xB3\xD1\x80\xD0\xB0 \xD0\xB1\xD1\x8B\xD0\xBB\xD0\xB0 \xD1\x81\xD0\xB2\xD1\x91\xD1\x80\xD0\xBD\xD1\x83\xD1\x82\xD0\xB0, \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4 \xD0\xBF\xD1\x80\xD0\xBE\xD0\xBF\xD1\x83\xD1\x89\xD0\xB5\xD0\xBD. \xD0\x9F\xD0\xBE\xD0\xB3\xD1\x80\xD0\xB5\xD1\x88\xD0\xBD\xD0\xBE\xD1\x81\xD1\x82\xD1\x8C (2 \xD0\xBC\xD0\xB8\xD0\xBD) \xD0\xBF\xD1\x80\xD0\xB5\xD0\xB2\xD1\x8B\xD1\x88\xD0\xB5\xD0\xBD\xD0\xB0.").c_str());
				});
				// Recalculate next report time
				float intervalSec = (float)activePatrol.intervalMin * 60.0f;
				if (intervalSec > 0.0f) {
					float elapsed = activePatrol.totalElapsedSec;
					float nextReportSec = ceil(elapsed / intervalSec) * intervalSec;
					if (nextReportSec <= elapsed) nextReportSec += intervalSec;
					activePatrol.timeRemainingSec = nextReportSec - elapsed;
				}
			} else {
				if (wasMinimized && overdueSec > 0) {
					RunOnMainThread([]() {
				...
				activePatrol.currentStage = 1; // PROCESS
				ExecutePatrolReport();
			}
		}
	}
	...
	int totalMins = 0;
	int totalSecs = 0;
	bool hasTotalTime = activePatrol.totalMin > 0;
	if (hasTotalTime) {
		float totalRem = (activePatrol.totalMin * 60.0f) - activePatrol.totalElapsedSec;
		if (totalRem < 0) totalRem = 0;
		totalMins = (int)totalRem / 60;
		totalSecs = (int)totalRem % 60;
	}
	char totBuf[16]; sprintf_s(totBuf, "%02d:%02d", totalMins, totalSecs);

	float timerY = pwY + HEADER_H + 19.0f;
	float leftAreaW = 165.0f;

	if (hasTotalTime) {
		// Draw side-by-side
		float b1x = pwX + leftAreaW * 0.28f;
		float b2x = pwX + leftAreaW * 0.72f;

		float ntW = fontSegoeBlack32->CalcTextSizeA(30.0f, FLT_MAX, 0.0f, tBuf).x;
		dl->AddText(fontSegoeBlack32, 30.0f, ImVec2(b1x - ntW*0.5f, timerY + 4), C_WHITE, tBuf);
		float nsW = fontSegoeBold12->CalcTextSizeA(11.0f, FLT_MAX, 0.0f, "\xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94").x;
		dl->AddText(fontSegoeBold12, 11.0f, ImVec2(b1x - nsW*0.5f, timerY + 32), IM_COL32(107, 115, 127, 255), "\xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94");

		float ttW = fontSegoeBlack32->CalcTextSizeA(30.0f, FLT_MAX, 0.0f, totBuf).x;
		dl->AddText(fontSegoeBlack32, 30.0f, ImVec2(b2x - ttW*0.5f, timerY + 4), C_GOLD, totBuf);
		float tsW = fontSegoeBold12->CalcTextSizeA(11.0f, FLT_MAX, 0.0f, "\xD0\x9A\xD0\x9E\xD0\x9D\xD0\x95\xD0\xA6").x;
		dl->AddText(fontSegoeBold12, 11.0f, ImVec2(b2x - tsW*0.5f, timerY + 32), IM_COL32(107, 115, 127, 255), "\xD0\x9A\xD0\x9E\xD0\x9D\xD0\x95\xD0\xA6");
		
		// Add a vertical divider between them
		dl->AddLine(ImVec2(pwX + leftAreaW * 0.5f, timerY + 6), ImVec2(pwX + leftAreaW * 0.5f, timerY + 38), C_BORDER, 1.0f);
	} else {
		// Just the single big timer
		float ntW = fontSegoeBlack32->CalcTextSizeA(36.0f, FLT_MAX, 0.0f, tBuf).x;
		dl->AddText(fontSegoeBlack32, 36.0f, ImVec2(pwX + leftAreaW*0.5f - ntW*0.5f, timerY), C_WHITE, tBuf);
		const char* sub = "\xD0\x94\xD0\x9E \xD0\xA1\xD0\x9B\xD0\x95\xD0\x94\xD0\xA3\xD0\xAE\xD0\xA9\xD0\x95\xD0\x93\xD0\x9E \xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94\xD0\x90";
		float nsW = fontSegoeBold12->CalcTextSizeA(11.0f, FLT_MAX, 0.0f, sub).x;
		dl->AddText(fontSegoeBold12, 11.0f, ImVec2(pwX + leftAreaW*0.5f - nsW*0.5f, timerY + 34), IM_COL32(107, 115, 127, 255), sub);
	}

	// Stats and Action box right side
	float vbx = pwX + 175, vby = pwY + HEADER_H + 11;
	dl->AddRectFilled(ImVec2(vbx, vby), ImVec2(vbx + 195, pwY + h - 14), C_BOX, 4.0f);
	dl->AddRect(ImVec2(vbx, vby), ImVec2(vbx + 195, pwY + h - 14), C_BORDER, 4.0f, 0, 1.5f);

	float vty = vby + 5;
	char sentBuf[64]; snprintf(sentBuf, sizeof(sentBuf), "\xD0\x9E\xD0\xA2\xD0\x9F\xD0\xA0\xD0\x90\xD0\x92\xD0\x9B\xD0\x95\xD0\x9D\xD0\x9E: %d", activePatrol.reportsSent);
	char sentBuf2[128]; snprintf(sentBuf2, sizeof(sentBuf2), "%s \xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94\xD0\x9E\xD0\x92", sentBuf);
	dl->AddText(fontSegoeBold14, 11.0f, ImVec2(vbx + 8, vty), IM_COL32(139, 148, 158, 255), sentBuf2);
	vty += 15;
	
	if (activePatrol.totalMin > 0 && activePatrol.intervalMin > 0) {
		int totalReports = (activePatrol.totalMin / activePatrol.intervalMin) + 1;
		int rem = totalReports - activePatrol.reportsSent;
		if (rem < 0) rem = 0;
		char remBuf[64]; snprintf(remBuf, sizeof(remBuf), "\xD0\x9E\xD0\xA1\xD0\xA2\xD0\x90\xD0\x9B\xD0\x9E\xD0\xA1\xD0\xAC: %d", rem);
		char remBuf2[128]; snprintf(remBuf2, sizeof(remBuf2), "%s \xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94\xD0\x9E\xD0\x92", remBuf);
		dl->AddText(fontSegoeBold14, 11.0f, ImVec2(vbx + 8, vty), IM_COL32(139, 148, 158, 255), remBuf2);
		vty += 15;
	}

	dl->AddLine(ImVec2(vbx, vty + 4), ImVec2(vbx + 195, vty + 4), C_BORDER, 1.0f);
	vty += 8;

	const char* actionTxt = "\xD0\x9E\xD0\xA7\xD0\x95\xD0\xA0\xD0\x95\xD0\x94\xD0\x9D\xD0\x9E\xD0\x99 \xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94";
	bool isLastReport = false;
	if (activePatrol.totalMin > 0) {
		if (activePatrol.intervalMin > 0) {
			int totalReports = (activePatrol.totalMin / activePatrol.intervalMin) + 1;
			if ((totalReports - activePatrol.reportsSent) <= 1) isLastReport = true;
		} else if ((activePatrol.totalElapsedSec + activePatrol.timeRemainingSec) + 1.0f >= activePatrol.totalMin * 60.0f) {
			isLastReport = true;
		}
	}
	if (isLastReport) {
		actionTxt = "\xD0\x97\xD0\x90\xD0\x92\xD0\x95\xD0\xA0\xD0\xA8\xD0\x95\xD0\x9D\xD0\x98\xD0\x95 \xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94\xD0\x9E\xD0\x92";
	}
	char actionBuf[128];
...
void Gui::RenderPatrolsTab(ImDrawList* dl, ImVec2 o) {
    if (patrols.empty()) selectedPatrolIndex = 0;
    ...
    // Empty state (same style as laws tab)
    if (patrols.empty()) {
        // Center icon: shield with clock
        float icx = o.x + 350, icy = o.y + 230;
        // Shield outline
        dl->AddLine(ImVec2(icx, icy-40), ImVec2(icx+30, icy-25), C_BORDER, 4.0f);
        dl->AddLine(ImVec2(icx+30, icy-25), ImVec2(icx+30, icy+10), C_BORDER, 4.0f);
        dl->AddLine(ImVec2(icx+30, icy+10), ImVec2(icx, icy+40), C_BORDER, 4.0f);
        dl->AddLine(ImVec2(icx, icy+40), ImVec2(icx-30, icy+10), C_BORDER, 4.0f);
        dl->AddLine(ImVec2(icx-30, icy+10), ImVec2(icx-30, icy-25), C_BORDER, 4.0f);
        dl->AddLine(ImVec2(icx-30, icy-25), ImVec2(icx, icy-40), C_BORDER, 4.0f);
        // Clock inside
        dl->AddCircle(ImVec2(icx, icy), 12.0f, C_LINE, 12, 3.0f);
        dl->AddLine(ImVec2(icx, icy-8), ImVec2(icx, icy), C_LINE, 3.0f);
        dl->AddLine(ImVec2(icx, icy), ImVec2(icx+6, icy+3), C_LINE, 3.0f);

        // Title text
        const char* emtTxt = "\xD0\x90\xD0\x92\xD0\xA2\xD0\x9E-\xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94\xD0\xAB \xD0\x9D\xD0\x95 \xD0\x9D\xD0\x90\xD0\xA1\xD0\xA2\xD0\xA0\xD0\x9E\xD0\x95\xD0\x9D\xD0\xAB";
        ...
        const char* emtSub = "\xD0\x9E\xD1\x82\xD0\xBA\xD1\x80\xD0\xBE\xD0\xB9\xD1\x82\xD0\xB5 \xD0\xBB\xD0\xB0\xD1\x83\xD0\xBD\xD1\x87\xD0\xB5\xD1\x80 \xD0\xB8 \xD0\xBD\xD0\xB0\xD1\x81\xD1\x82\xD1\x80\xD0\xBE\xD0\xB9\xD1\x82\xD0\xB5 \xD1\x81\xD1\x86\xD0\xB5\xD0\xBD\xD0\xB0\xD1\x80\xD0\xB8\xD0\xB8 \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4\xD0\xBE\xD0\xB2.";
    ...
    // СПИСОК ДОКЛАДОВ
    dl->AddText(fontSegoeBold14, 12.0f, ImVec2(o.x + 28, o.y + 76), IM_COL32(139, 148, 158, 255),
        "\xD0\xA1\xD0\x9F\xD0\x98\xD0\xA1\xD0\x9E\xD0\x9A \xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94\xD0\x9E\xD0\x92");

    // SVG separator: exactly like Binder tab
    dl->AddLine(ImVec2(o.x + 30, o.y + 100), ImVec2(o.x + 150, o.y + 100), C_BORDER, 1.5f);

    // Sidebar items (shifted down to avoid overlap)
    ImGui::SetCursorScreenPos(ImVec2(sx + 10, o.y + 110));
    // Provide proper window context to avoid "Debug" artifact
    ImGui::PushStyleColor(ImGuiCol_WindowBg, 0);
    ImGui::PushStyleColor(ImGuiCol_Border, 0);
    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0, 4));
    ImGui::BeginChild("##PatrolsSidebar", ImVec2(sbW - 20, sbH - 45), false, ImGuiWindowFlags_NoBackground);
    for (int i = 0; i < (int)patrols.size(); i++) {
        bool isSel = (selectedPatrolIndex == i);
        ImVec2 cp = ImGui::GetCursorScreenPos();
        ImGui::InvisibleButton((std::string("##p") + std::to_string(i)).c_str(), ImVec2(120, 28));
        if (ImGui::IsItemClicked()) {
            if (activePatrol.active && selectedPatrolIndex != i) {
                overlayErrorMsg = "\xD0\xA1\xD0\xBD\xD0\xB0\xD1\x87\xD0\xB0\xD0\xBB\xD0\xB0 \xD0\xB7\xD0\xB0\xD0\xB2\xD0\xB5\xD1\x80\xD1\x88\xD0\xB8\xD1\x82\xD0\xB5 \xD1\x82\xD0\xB5\xD0\xBA\xD1\x83\xD1\x89\xD0\xB8\xD0\xB9 \xD0\xBF\xD1\x80\xD0\xBE\xD1\x86\xD0\xB5\xD1\x81\xD1\x81 \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4\xD0\xBE\xD0\xB2.";
                overlayErrorColor = IM_COL32(239, 68, 68, 255);
                overlayErrorTimer = 3.0f;
            } else {
        ...
        if (isSel) {
            cdl->AddRectFilled(cp, ImVec2(cp.x + 120, cp.y + 28), C_GOLD_BG, 6.0f);
            cdl->AddRectFilled(cp, ImVec2(cp.x + 2, cp.y + 28), C_GOLD, 2.0f);
        } else if (hover) {
            cdl->AddRectFilled(cp, ImVec2(cp.x + 120, cp.y + 28), IM_COL32(255, 255, 255, 10), 6.0f);
        }
        ImU32 textCol = isSel ? C_GOLD : C_WHITE;
        cdl->AddText(fontSegoeBold14, 11.0f, ImVec2(cp.x + 10, cp.y + 6), textCol, patrols[i].name.c_str());
    }
    ImGui::EndChild();
    ImGui::PopStyleVar();
    ImGui::PopStyleColor(2);

    // SVG right panel: rect x=170 y=65 w=510 h=347
    ...
    // SVG: "ФОРМАТ ДОКЛАДА" 11px bold gray
    dl->AddText(fontSegoeBold14, 12.0f, ImVec2(px + 20, o.y + 76),
    ...
    // SVG format box: 470x86
    float fx = px + 20, fy = o.y + 96;
    dl->AddRectFilled(ImVec2(fx, fy), ImVec2(fx + 470, fy + 86), C_INPUT, 4.0f);
    dl->AddRect(ImVec2(fx, fy), ImVec2(fx + 470, fy + 86), C_BORDER, 4.0f, 0, 1.0f);

    // Format text lines: scrollable box for long texts
    ImGui::SetCursorScreenPos(ImVec2(fx + 1, fy + 1));
    ...
    ImGui::PushStyleColor(ImGuiCol_ScrollbarGrabActive, C_GOLD);
    ImGui::PushStyleVar(ImGuiStyleVar_ScrollbarSize, 6.0f);
    ImGui::BeginChild("##FormatChild", ImVec2(468, 84), false, ImGuiWindowFlags_NoBackground);
    
    ImDrawList* cdl = ImGui::GetWindowDrawList();
    float cx = fx + 10;
    
    auto drawFmtLine = [&](const char* label, const char* text) {
        ImVec2 cp = ImGui::GetCursorScreenPos();
    ...
    drawFmtLine("\xD0\x9D\xD0\xB0\xD1\x87\xD0\xB0\xD0\xBB\xD0\xBE:", p.startText.c_str());
    drawFmtLine("\xD0\x92 \xD0\xBF\xD1\x80\xD0\xBE\xD1\x86\xD0\xB5\xD1\x81\xD1\x81\xD0\xB5:", p.processText.c_str());
    drawFmtLine("\xD0\x9E\xD0\xBA\xD0\xBE\xD0\xBD\xD1\x87\xD0\xB0\xD0\xBD\xD0\xB8\xD0\xB5:", p.endText.c_str());
    ImGui::Dummy(ImVec2(10, 2));
    ...
    // Variables section
    if (!vars.empty()) {
        dl->AddText(fontSegoeBold14, 12.0f, ImVec2(px + 20, o.y + 190),
            IM_COL32(139, 148, 158, 255),
        ...
        float startY = (vars.size() <= 2) ? (o.y + 226) : (o.y + 211);
        ...
        for (size_t i = 0; i < vars.size() && i < 4; i++) {
            ...
            char varBuf[256] = "";
            if (editVariables.count(vars[i])) strcpy_s(varBuf, editVariables[vars[i]].c_str());
            else if (activePatrol.variables.count(vars[i])) {
                strcpy_s(varBuf, activePatrol.variables[vars[i]].c_str());
    ...
    // Separator at y=270
    dl->AddLine(ImVec2(px, o.y + 270), ImVec2(px + panW, o.y + 270), C_BORDER, 1.0f);

    // Total Time (Left)
    dl->AddText(fontSegoeBold14, 13.0f, ImVec2(px + 20, o.y + 285),
        IM_COL32(139, 148, 158, 255),
        "\xD0\x9E\xD0\x91\xD0\xA9\xD0\x95\xD0\x95 \xD0\x92\xD0\xA0\xD0\x95\xD0\x9C\xD0\xAF:");

    static char editTotalTimeBuf[8] = "";
    ...
    if (lastSelIdx != selectedPatrolIndex) {
        ...
        editTotalTimeBuf[0] = '\0'; // Reset to empty when switching
    }

    float totX = px + 120, totY = o.y + 280;
    dl->AddRectFilled(ImVec2(totX, totY), ImVec2(totX + 40, totY + 22), C_INPUT, 4.0f);
    dl->AddRect(ImVec2(totX, totY), ImVec2(totX + 40, totY + 22), C_BORDER, 4.0f, 0, 1.0f);

    ImGui::SetCursorScreenPos(ImVec2(totX + 2, totY + 1));
    ImGui::PushItemWidth(36);
    ImGui::PushStyleColor(ImGuiCol_FrameBg, IM_COL32(0, 0, 0, 0));
    ImGui::PushStyleColor(ImGuiCol_Border, IM_COL32(0, 0, 0, 0));
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(4, 2));
    ImGui::PushFont(fontSegoeBold14);
    if (activePatrol.active) ImGui::BeginDisabled(true);
    ImGui::InputText("##patrolTotalTime", editTotalTimeBuf, 8, ImGuiInputTextFlags_CharsDecimal);
    if (activePatrol.active) ImGui::EndDisabled();
    ImGui::PopFont();
    ImGui::PopStyleVar();
    ImGui::PopStyleColor(2);
    ImGui::PopItemWidth();

    dl->AddText(fontSegoeRegular13, 12.0f, ImVec2(totX + 45, o.y + 285),
        IM_COL32(139, 148, 158, 255), "\xD0\xBC\xD0\xB8\xD0\xBD.");

    // Interval (Right)
    float intLabelX = px + 210;
    dl->AddText(fontSegoeBold14, 13.0f, ImVec2(intLabelX, o.y + 285),
        IM_COL32(139, 148, 158, 255),
        "\xD0\x98\xD0\x9D\xD0\xA2\xD0\x95\xD0\xA0\xD0\x92\xD0\x90\xD0\x9B \xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94\xD0\x90:");

    float intX = intLabelX + 130, intY = o.y + 280;
    dl->AddRectFilled(ImVec2(intX, intY), ImVec2(intX + 40, intY + 22), C_INPUT, 4.0f);
    dl->AddRect(ImVec2(intX, intY), ImVec2(intX + 40, intY + 22), C_BORDER, 4.0f, 0, 1.0f);
    ...
    dl->AddRectFilled(ImVec2(cbX, cbY), ImVec2(cbX + 14, cbY + 14), C_INPUT, 2.0f);
    dl->AddRect(ImVec2(cbX, cbY), ImVec2(cbX + 14, cbY + 14), C_BORDER, 2.0f, 0, 1.0f);
    ...
    dl->AddText(fontSegoeBold14, 13.0f, ImVec2(cbX + 25, cbY),
        IM_COL32(139, 148, 158, 255),
        "\xD0\x90\xD0\xB2\xD1\x82\xD0\xBE\xD0\xBC\xD0\xB0\xD1\x82\xD0\xB8\xD1\x87\xD0\xB5\xD1\x81\xD0\xBA\xD0\xB8 \xD0\xB4\xD0\xB5\xD0\xBB\xD0\xB0\xD1\x82\xD1\x8C \xD1\x81\xD0\xBA\xD1\x80\xD0\xB8\xD0\xBD\xD1\x88\xD0\xBE\xD1\x82 (/c 60 + F8)");
	...
	if (activePatrol.active) {
		if (activePatrol.data.name != p.name) {
			needsUpdate = true;
		} else {
			for (const auto& v : vars) {
				if (editVariables.count(v) && activePatrol.variables[v] != editVariables[v]) {
					needsUpdate = true;
					break;
				}
			}
			if (activePatrol.intervalMin != editInterval) needsUpdate = true;
			int editTot = editTotalTimeBuf[0] ? atoi(editTotalTimeBuf) : 0;
			if (activePatrol.totalMin != editTot) needsUpdate = true;
		}
	}
    ...
    if (activePatrol.active) {
        if (needsUpdate) {
            dl->AddRectFilled(ImVec2(px + 20, btnY), ImVec2(px + 490, btnY + 40),
                startHover ? IM_COL32(46, 204, 113, 70) : IM_COL32(46, 204, 113, 38), 4.0f);
            dl->AddRect(ImVec2(px + 20, btnY), ImVec2(px + 490, btnY + 40),
                IM_COL32(46, 204, 113, 255), 4.0f, 0, 1.5f);
            const char* sTxt = "\xD0\x9E\xD0\x91\xD0\x9D\xD0\x9E\xD0\x92\xD0\x98\xD0\xA2\xD0\xAC \xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94";
            ImVec2 sSz = fontSegoeBold14->CalcTextSizeA(13.0f, 3.402823466e+38F, 0.0f, sTxt);
            float totalW = 16.0f + 8.0f + sSz.x;
            float startX = px + 20 + (470 - totalW) / 2;
            
            dl->AddCircle(ImVec2(startX + 8, btnY + 20), 5.0f, IM_COL32(46, 204, 113, 255), 12, 1.5f);
            dl->AddTriangleFilled(ImVec2(startX + 10, btnY + 12), ImVec2(startX + 14, btnY + 15), ImVec2(startX + 10, btnY + 18), IM_COL32(46, 204, 113, 255));
            
            dl->AddText(fontSegoeBold14, 13.0f, ImVec2(startX + 24.0f, btnY + (40 - sSz.y) / 2), IM_COL32(46, 204, 113, 255), sTxt);
            
            if (startBtn) {
                for (const auto& kv : editVariables) {
                    activePatrol.variables[kv.first] = kv.second;
                }
            }
        } else {
            dl->AddRectFilled(ImVec2(px + 20, btnY), ImVec2(px + 490, btnY + 40),
            ...
            dl->AddRect(ImVec2(px + 20, btnY), ImVec2(px + 490, btnY + 40),
                IM_COL32(231, 76, 60, 255), 4.0f, 0, 1.5f);
							const char* sTxt = "\xD0\x97\xD0\x90\xD0\x92\xD0\x95\xD0\xA0\xD0\xA8\xD0\x98\xD0\xA2\xD0\xAC \xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94";
            ImVec2 sSz = fontSegoeBold14->CalcTextSizeA(13.0f, 3.402823466e+38F, 0.0f, sTxt);
            float totalW = 12.0f + 8.0f + sSz.x;
            float startX = px + 20 + (470 - totalW) / 2;
            dl->AddRectFilled(ImVec2(startX, btnY + 14), ImVec2(startX + 12, btnY + 26), IM_COL32(231, 76, 60, 255));
							dl->AddText(fontSegoeBold14, 13.0f, ImVec2(startX + 20.0f, btnY + (40 - sSz.y) / 2), IM_COL32(231, 76, 60, 255), "\xD0\x97\xD0\x90\xD0\x92\xD0\x95\xD0\xA0\xD0\xA8\xD0\x98\xD0\xA2\xD0\xAC \xD0\x94\xD0\x9E\xD0\x9A\xD0\x9B\xD0\x90\xD0\x94");
        ...
        if (startBtn) {
								...
								activePatrol.data = p;
								activePatrol.variables.clear();
								for (const auto& kv : editVariables) {
									activePatrol.variables[kv.first] = kv.second;
								}
								activePatrol.currentStage = 0;
            activePatrol.intervalMin = editInterval;
            activePatrol.totalMin = editTotalTimeBuf[0] ? atoi(editTotalTimeBuf) : 0;
            activePatrol.totalElapsedSec = 0.0f;
            ...
            activePatrol.lastTickTime = GetTickCount64();
            
            // Trigger first report immediately
            // System notifications
            RunOnMainThread([p, totalMin = activePatrol.totalMin]() {
                extern void AddLocalSAMPMessage(const char*);
                char msgBuf[512];
                snprintf(msgBuf, sizeof(msgBuf), "{D2A65E}[DURAN HELPER] {FFFFFF}\xD0\x97\xD0\xB0\xD0\xBF\xD1\x83\xD1\x89\xD0\xB5\xD0\xBD \xD1\x81\xD1\x86\xD0\xB5\xD0\xBD\xD0\xB0\xD1\x80\xD0\xB8\xD0\xB9 \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4\xD0\xBE\xD0\xB2: \"%s\". \xD0\xA7\xD1\x82\xD0\xBE\xD0\xB1\xD1\x8B \xD0\xB7\xD0\xB0\xD0\xB2\xD0\xB5\xD1\x80\xD1\x88\xD0\xB8\xD1\x82\xD1\x8C \xD1\x80\xD0\xB0\xD0\xBD\xD1\x8C\xD1\x88\xD0\xB5, \xD0\xB7\xD0\xB0\xD0\xB9\xD0\xB4\xD0\xB8\xD1\x82\xD0\xB5 \xD0\xB2\xD0\xBE \xD0\xB2\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4\xD0\xBA\xD1\x83 \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4\xD0\xBE\xD0\xB2.", p.name.c_str());
                std::string cp1251_1 = UTF8ToCP1251(msgBuf);
                AddLocalSAMPMessage(cp1251_1.c_str());
                
                time_t t = time(NULL);
                struct tm tmBuf;
                localtime_s(&tmBuf, &t);
                char startTime[16];
                snprintf(startTime, sizeof(startTime), "%02d:%02d", tmBuf.tm_hour, tmBuf.tm_min);
                
                if (totalMin > 0) {
                    t += totalMin * 60;
                    localtime_s(&tmBuf, &t);
                    char endTime[16];
                    snprintf(endTime, sizeof(endTime), "%02d:%02d", tmBuf.tm_hour, tmBuf.tm_min);
                    snprintf(msgBuf, sizeof(msgBuf), "{D2A65E}[DURAN HELPER] {FFFFFF}\xD0\x92\xD1\x80\xD0\xB5\xD0\xBC\xD1\x8F \xD0\xBD\xD0\xB0\xD1\x87\xD0\xB0\xD0\xBB\xD0\xB0 \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4\xD0\xBE\xD0\xB2: %s. \xD0\x92\xD1\x80\xD0\xB5\xD0\xBC\xD1\x8F \xD0\xBE\xD0\xBA\xD0\xBE\xD0\xBD\xD1\x87\xD0\xB0\xD0\xBD\xD0\xB8\xD1\x8F \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4\xD0\xBE\xD0\xB2: %s", startTime, endTime);
                } else {
                    snprintf(msgBuf, sizeof(msgBuf), "{D2A65E}[DURAN HELPER] {FFFFFF}\xD0\x92\xD1\x80\xD0\xB5\xD0\xBC\xD1\x8F \xD0\xBD\xD0\xB0\xD1\x87\xD0\xB0\xD0\xBB\xD0\xB0 \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4\xD0\xBE\xD0\xB2: %s. \xD0\x92\xD1\x80\xD0\xB5\xD0\xBC\xD1\x8F \xD0\xBE\xD0\xBA\xD0\xBE\xD0\xBD\xD1\x87\xD0\xB0\xD0\xBD\xD0\xB8\xD1\x8F \xD0\xB4\xD0\xBE\xD0\xBA\xD0\xBB\xD0\xB0\xD0\xB4\xD0\xBE\xD0\xB2: \xD0\x91\xD0\xB5\xD1\x81\xD0\xBA\xD0\xBE\xD0\xBD\xD0\xB5\xD1\x87\xD0\xBD\xD0\xBE", startTime);
                }
                std::string cp1251_2 = UTF8ToCP1251(msgBuf);
                AddLocalSAMPMessage(cp1251_2.c_str());
            });
...
void Gui::ExecutePatrolReport() {
    if (!activePatrol.active) return;
	activePatrol.reportsSent++;
    
    std::string text;
    if (activePatrol.currentStage == 0) text = activePatrol.data.startText;
    ...
    for (const auto& kv : activePatrol.variables) {
        ...
        while ((pos = text.find(tag, pos)) != std::string::npos) {
            text.replace(pos, tag.length(), kv.second);
    ...
    // Replace time if any
    size_t tPos = 0;
    std::string timeTag = "*\xD0\x92\xD0\xA0\xD0\x95\xD0\x9C\xD0\xAF*"; // *ÐÐ ÐÐÐ¯*
    std::string curTime = GetCurrentDateTime().substr(11, 5); // Assuming HH:MM
    while ((tPos = text.find(timeTag, tPos)) != std::string::npos) {
        text.replace(tPos, timeTag.length(), curTime);
        tPos += curTime.length();
    }
    
    std::string cp1251 = UTF8ToCP1251(text.c_str());

    RunOnMainThread([cp1251]() {
        extern void SendSAMPMessage(const char*);
        SendSAMPMessage(cp1251.c_str());
        
        if (Gui::activePatrol.autoSend) {
            extern void StartAutoScreenshot();
            StartAutoScreenshot();
        }
    });

    if (activePatrol.currentStage < 2) {
        float intervalSec = (float)activePatrol.intervalMin * 60.0f;
		if (intervalSec > 0.0f) {
			float elapsed = activePatrol.totalElapsedSec;
			float nextReportSec = ceil(elapsed / intervalSec) * intervalSec;
			...
			activePatrol.timeRemainingSec = 0.0f;
		}
    }
}
